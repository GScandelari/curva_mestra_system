...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!new_consultant_id) {
      return NextResponse.json({ error: 'new_consultant_id é obrigatório' }, { status: 400 });
    }
    ...
    const newConsultantDoc = await adminDb.collection('consultants').doc(new_consultant_id).get();

    if (!newConsultantDoc.exists) {
      return NextResponse.json({ error: 'Novo consultor não encontrado' }, { status: 404 });
    }

    const newConsultantData = newConsultantDoc.data();

    if (newConsultantData?.status !== 'active') {
      return NextResponse.json({ error: 'Novo consultor não está ativo' }, { status: 400 });
    }
    ...
    if (oldConsultantId) {
      const oldConsultantDoc = await adminDb.collection('consultants').doc(oldConsultantId).get();
