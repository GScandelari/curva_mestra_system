  ...
  if (error || !solicitacao) {
    return (
      <div className="container py-8">
        ...
        <Button variant="outline" className="mt-4" onClick={() => router.push('/clinic/requests')}>
  ...
  return (
    <div className="container py-8">
      <div className="space-y-6">
        {/* Header */}
        <div>
          <Button variant="ghost" onClick={() => router.push('/clinic/requests')} className="mb-4">
            <ArrowLeft className="mr-2 h-4 w-4" />
            Voltar
          </Button>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-muted-foreground">Detalhes do consumo de produtos</p>
            </div>
            <div className="flex items-center gap-2">{getStatusBadge(solicitacao.status)}</div>
          </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Resumo financeiro dos produtos consumidos</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="p-4 border rounded-lg">
                ...
                <p className="text-2xl font-bold">{solicitacao.produtos_solicitados.length}</p>
              </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">{solicitacao.created_by_name || 'N/A'}</p>
              </div>
              <div>
                <Label className="text-muted-foreground">Criado em</Label>
                <p className="font-medium">{formatTimestamp(solicitacao.created_at)}</p>
              </div>
              <div>
                <Label className="text-muted-foreground">Última atualização</Label>
                <p className="font-medium">{formatTimestamp(solicitacao.updated_at)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {solicitacao.status === 'cancelada' && (
          <Alert>
            ...
            <AlertDescription>
              Este procedimento foi cancelado. O estoque foi ajustado de acordo com o status
              anterior.
            </AlertDescription>
          </Alert>
        )}
