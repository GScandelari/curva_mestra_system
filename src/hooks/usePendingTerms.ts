...
export function usePendingTerms() {
  ...
  async function checkPendingTerms() {
    ...
    try {
      ...
      docs.forEach((doc) => {
        console.log(
          `  - ${doc.title} (v${doc.version}): reg=${doc.required_for_registration}, exist=${doc.required_for_existing_users}`
        );
      });
      ...
      const pending = docs.filter((doc) => {
        ...
        console.log(
          `  - ${doc.title}: aceito=${acceptedVersion}, versão atual=${doc.version}, pendente=${isPending}`
        );
        return isPending;
      });
