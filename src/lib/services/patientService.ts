...
export async function getPatientById(tenantId: string, patientId: string): Promise<Patient | null> {
  try {
    const patientDoc = await getDoc(doc(db, 'tenants', tenantId, 'patients', patientId));
 ...
 */
export async function getPatientByCode(tenantId: string, codigo: string): Promise<Patient | null> {
  try {
    const q = query(
...
export async function listPatients(
  ...
  try {
    ...
    let q = query(collection(db, 'tenants', tenantId, 'patients'), orderBy('created_at', 'desc'));
...
export async function updatePatient(
  ...
  try {
    ...
    if (updates.nome !== undefined && updates.nome.trim() !== currentPatient.nome) {
      ...
      changes.push({
        field: 'nome',
        old_value: currentPatient.nome,
        new_value: updates.nome.trim(),
      });
    }
    if (
      updates.telefone !== undefined &&
      (updates.telefone?.trim() || null) !== (currentPatient.telefone || null)
    ) {
      updateData.telefone = updates.telefone?.trim() || null;
      changes.push({
        field: 'telefone',
        old_value: currentPatient.telefone,
        new_value: updates.telefone?.trim() || null,
      });
    }
    if (
      updates.email !== undefined &&
      (updates.email?.trim().toLowerCase() || null) !== (currentPatient.email || null)
    ) {
      updateData.email = updates.email?.trim().toLowerCase() || null;
      changes.push({
        field: 'email',
        old_value: currentPatient.email,
        new_value: updates.email?.trim().toLowerCase() || null,
      });
    }
    if (
      updates.data_nascimento !== undefined &&
      (updates.data_nascimento?.trim() || null) !== (currentPatient.data_nascimento || null)
    ) {
      updateData.data_nascimento = updates.data_nascimento?.trim() || null;
      changes.push({
        field: 'data_nascimento',
        old_value: currentPatient.data_nascimento,
        new_value: updates.data_nascimento?.trim() || null,
      });
    }
    ...
    if (
      updates.observacoes !== undefined &&
      (updates.observacoes?.trim() || null) !== (currentPatient.observacoes || null)
    ) {
      updateData.observacoes = updates.observacoes?.trim() || null;
      changes.push({
        field: 'observacoes',
        old_value: currentPatient.observacoes,
        new_value: updates.observacoes?.trim() || null,
      });
    }
    ...
    if (changes.length > 0 && userId && userName) {
      await addDoc(collection(db, 'tenants', tenantId, 'patient_edit_logs'), {
...
export async function getPatientsStats(tenantId: string): Promise<{
  ...
  try {
    ...
    const novos_mes = patients.filter((p) => {
      ...
      return createdAt.getMonth() === mesAtual && createdAt.getFullYear() === anoAtual;
    }).length;
 ...
 */
export async function getPatientHistory(tenantId: string, patientCode: string): Promise<any[]> {
  try {
    const solicitacoesRef = collection(db, 'tenants', tenantId, 'solicitacoes');
    const q = query(
      solicitacoesRef,
...
export async function getPatientEditLogs(
  ...
  try {
    ...
    const q = query(logsRef, where('patient_id', '==', patientId), orderBy('edited_at', 'desc'));
