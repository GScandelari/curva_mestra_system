...
exports.logActivity = exports.setCostThresholds = exports.getCostReport = exports.collectDailyUsageMetrics = void 0;
 ...
 */
exports.collectDailyUsageMetrics = functions.pubsub
    ...
    try {
        ...
        await firestore.collection('system').doc('costs').collection('daily_metrics').doc(dateStr).set(Object.assign(Object.assign({}, metrics), { date: dateStr, estimatedCost, timestamp: admin.firestore.FieldValue.serverTimestamp() }));
 ...
 */
async function collectUsageMetrics(date) {
 ...
 */
function calculateEstimatedCost(metrics) {
    ...
    const costs = {
        firestore: (metrics.firestoreReads * pricing.firestoreRead +
            metrics.firestoreWrites * pricing.firestoreWrite +
            metrics.firestoreDeletes * pricing.firestoreDelete),
        functions: (metrics.functionsInvocations * pricing.functionsInvocation +
 ...
 * Verificar se excedeu limites de custo
 */
async function checkCostThresholds(dailyCost, date) {
    const thresholds = {
 ...
 */
async function calculatePeriodCost(startDate, endDate) {
    try {
 ...
 */
async function createCostAlert(period, actualCost, threshold, date) {
    try {
 ...
 */
exports.getCostReport = functions.https.onCall(async (data, context) => {
    var _a, _b;
    ...
    try {
        ...
        return {
            ...
            projections,
            period: {
                startDate: startDate || ((_a = metrics[metrics.length - 1]) === null || _a === void 0 ? void 0 : _a.date),
                endDate: endDate || ((_b = metrics[0]) === null || _b === void 0 ? void 0 : _b.date),
                days: metrics.length
            }
        };
    }
 ...
 */
exports.setCostThresholds = functions.https.onCall(async (data, context) => {
    // Verificar se é admin
    if (!context.auth || !context.auth.token.role || context.auth.token.role !== 'admin') {
...
exports.logActivity = functions.https.onCall(async (data, context) => {
    ...
    try {
        await firestore.collection('system').doc('activity').collection('logs').add({
            ...
            clinicId: (_a = context.auth.token) === null || _a === void 0 ? void 0 : _a.clinicId,
            timestamp: admin.firestore.FieldValue.serverTimestamp()
        });
    }
    catch (error) {
