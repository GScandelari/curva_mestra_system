 ...
 */
async function sendCriticalErrorAlert(errorId: string, errorReport: ErrorReport) {
  try {
 ...
 */
async function incrementErrorCounter(errorType: string, severity: string) {
...
export const generateDailyErrorReport = functions.pubsub
  ...
  .onRun(async (context) => {
    try {
      ...
      const totalErrors = errorCounts?.total || 0;
      ...
      await firestore.collection('system').doc('errors').collection('daily_reports').doc(yesterdayStr).set({
        ...
        criticalErrors: errorCounts?.total_critical || 0,
        highErrors: errorCounts?.total_high || 0,
        mediumErrors: errorCounts?.total_medium || 0,
        lowErrors: errorCounts?.total_low || 0,
 ...
 */
async function createHighErrorVolumeAlert(date: string, errorCounts: any) {
  const adminsSnapshot = await firestore.collectionGroup('users')
    .where('role', '==', 'admin')
    .get();
 ...
 */
async function createUnresolvedCriticalErrorsAlert(count: number) {
  const adminsSnapshot = await firestore.collectionGroup('users')
    .where('role', '==', 'admin')
    .get();
 ...
 */
export const resolveError = functions.https.onCall(async (data, context) => {
 ...
 */
export const getErrorStats = functions.https.onCall(async (data, context) => {
  // Verificar se é admin
  if (!context.auth || !context.auth.token.role || context.auth.token.role !== 'admin') {
