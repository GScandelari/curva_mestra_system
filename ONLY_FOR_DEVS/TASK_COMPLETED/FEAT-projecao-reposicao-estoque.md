...
| 1.3 | 23/09/2026 | Guilherme Scandelari | Task concluída — movida para TASK_COMPLETED. Todos os 9 steps do Plano de Implementação (Seção 7) implementados na branch `feat/uc52-projecao-reposicao-estoque`, a partir de `develop`: funções puras de cálculo (`projectionService.ts`), 21 testes unitários, orquestrador Firestore (`getReplenishmentProjections`), `ProjectionsView.tsx`, páginas de projeções para clínica e Consultor, cards de Dashboard (clínica e Consultor) e caderno Playwright (`tests/e2e/UC-52-consultar-projecao-de-reposicao-de-estoque.spec.ts`, gerado pelo qa-agent e revisado). Além da correção de RN-03 já registrada na v1.2 (cascata invertida de 90-60-30 para 30-60-90 dias, mais recente primeiro, por conta da aninhagem 30 dentro de 60 dentro de 90 tornar o fallback inalcançável na ordem original), um segundo problema real foi encontrado pelo qa-agent ao gerar o caderno Playwright (Step 9): `getReplenishmentProjections` lia `produto.codigo_produto` de cada item de `solicitacao.produtos_solicitados`, mas o campo gravado em `ProdutoSolicitado` é `produto_codigo` (`src/types/index.ts`, `solicitacaoService.ts`) — sem a correção, `eventsByCodigo` nunca era populado com a chave certa e todo produto aparecia como "Dados insuficientes para projeção" mesmo com consumo histórico real. Corrigido no commit `3137a42`, validado pelos 21 testes unitários e pelos 5 testes do caderno Playwright rodando contra o Firebase Emulator Suite (todos passando, 56.5s). Achado relacionado, fora do escopo desta branch e não corrigido aqui: o mesmo engano de nome de campo (`codigo_produto` em vez de `produto_codigo`) pré-existe em `src/lib/services/reportService.ts` (`generateConsumptionReport`, usado pelo relatório de consumo do UC-47) — recomendado abrir uma task de follow-up dedicada. Validação executada: `npm run lint` (0 erros), `npm run type-check` (0 erros), `npm run build` (OK, incluindo as novas rotas `/clinic/inventory/projections` e `/consultant/clinics/[tenantId]/projections`), `npm run test:coverage` (137/137 testes unitários passando). Validação manual do DoD (Seção 9: cards com dados reais, tela "Projeções Gerais" para clínica e Consultor, guard de acesso do Consultor a tenant não autorizado) ainda pendente, a ser feita após promoção da branch para `gscandelari_setup`. |
