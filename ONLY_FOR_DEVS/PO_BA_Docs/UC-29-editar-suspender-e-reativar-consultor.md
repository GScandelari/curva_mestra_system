...
- Se `email` for informado (o formulário sempre reenvia o e-mail atual, mesmo sem alteração — RN-05): `consultants/{id}.email` é atualizado **e** `adminAuth.updateUser(user_id, { email })` é chamado, alterando o e-mail de login no Firebase Auth. **[CORRIGIDO em v1.2.3, commit `001671b`]** Quando o e-mail **de fato muda** (`previousEmail !== emailLower` — a checagem usa o valor anterior salvo em `consultants`, então reenviar o mesmo e-mail sem alteração real, caso do RN-05, não dispara notificação nenhuma), o sistema enfileira em `email_queue` dois avisos idênticos (um para `previousEmail`, outro para o novo `emailLower`), informando da mudança e orientando o uso do novo e-mail no próximo login, com instrução de contatar o suporte caso a mudança não seja reconhecida (RN-03). Esse enfileiramento está em `try/catch` que só loga em caso de falha — não bloqueia a atualização em si.
...
- **Efeito real (corrigido):** o consultor não consegue mais logar (`active: false` bloqueia acesso, verificado por `ProtectedRoute`) nem acessar nenhuma clínica à qual tinha acesso — a suspensão agora bloqueia acesso de fato, ao contrário do comportamento anterior a 16/07/2026 (RN-01, corrigida). **[CORRIGIDO — commit `c3f18d4`]**: ao tentar logar com a conta suspensa, o consultor agora vê a mensagem amigável em PT-BR ("Esta conta foi desativada. Entre em contato com o suporte.") em vez da mensagem crua do SDK do Firebase — ver RN-09.
...
7. Como `email` está sempre presente no payload (o formulário sempre reenvia o valor atual do campo — RN-05), a API **sempre** entra no ramo de verificação de e-mail: consulta duplicidade em `consultants` (ignorando o próprio documento) e chama `adminAuth.updateUser(user_id, { email })` — que pode falhar com `auth/email-already-exists` se o e-mail já pertencer a outra conta Auth (ex.: um `clinic_admin`). **[Novo passo, v1.2.3, commit `001671b`]** Se o e-mail salvo anteriormente (`previousEmail`) for diferente do novo e-mail informado, a API enfileira em `email_queue` um aviso de troca para o e-mail antigo e outro para o novo (RN-03); se forem iguais (reenvio do mesmo valor, caso comum descrito em RN-05), nenhuma notificação é criada.
...
### 8f. [CORRIGIDO — commit `c3f18d4`] Consultor suspenso tenta logar (`/login`) — mensagem de erro agora traduzida
...
3. `signIn` (`src/hooks/useAuth.ts`) captura a exceção e retorna `{ success: false, error: error.code }` — **[CORRIGIDO]** antes retornava `error.message` (mensagem bruta do SDK).
4. `translateFirebaseError` (`src/app/(auth)/login/page.tsx`) — **[CORRIGIDO]** ganhou `if (error.includes('user-disabled')) { return 'Esta conta foi desativada. Entre em contato com o suporte.'; }`, adicionado antes do `default`.
5. Sistema exibe ao consultor: "Esta conta foi desativada. Entre em contato com o suporte." — mensagem amigável em PT-BR, em vez do texto cru e em inglês do SDK. Mesma correção aplicada simultaneamente em UC-36 (RN-10), mesma tela de login.

**Comportamento anterior (histórico, antes da correção):** o sistema exibia a mensagem crua e em inglês do SDK (ex.: `Firebase: The user account has been disabled by an administrator. (auth/user-disabled)`), pois `translateFirebaseError` não tinha nenhum tratamento para `user-disabled` e caía no `default`. Este code path só se tornara alcançável na prática após a correção de RN-01 (16/07/2026), que passou a desabilitar a conta de verdade no Firebase Auth.

### 8g. Falha ao enfileirar aviso de troca de e-mail (tolerante a falha)
1. `adminDb.collection('email_queue').add(...)` falha para um ou ambos os avisos (RN-03, v1.2.3, commit `001671b`).
2. Erro é capturado por `try/catch` (`console.warn`); a atualização do e-mail em si **não é revertida** — o consultor pode ficar sem ser avisado da troca, mesmo com o e-mail de login já alterado.
...
| RN-03 | **[CORRIGIDO em v1.2.3, commit `001671b`]** Editar o e-mail de um consultor já vinculado a clínicas **não quebra estruturalmente** o vínculo — `authorized_tenants` não referencia e-mail, então o acesso às clínicas permanece intacto. O risco de UX/suporte identificado (nenhuma notificação era enviada ao consultor quando seu e-mail de login mudava) foi corrigido: quando `previousEmail !== emailLower`, a API enfileira em `email_queue` (mesmo padrão de fila usado em outros fluxos do arquivo) uma notificação enviada tanto para o e-mail **antigo** quanto para o **novo**, avisando da mudança, orientando o uso do novo e-mail no próximo login, e instruindo a contatar o suporte caso a mudança não seja reconhecida. O envio está em `try/catch` que só loga em caso de falha (RN-03 tolerante — não bloqueia a atualização do e-mail em si). | Confirmado por diff do commit `001671b` em `src/app/api/consultants/[id]/route.ts` — bloco condicional `if (previousEmail && previousEmail !== emailLower)` com dois `adminDb.collection('email_queue').add(...)`. |
...
| RN-05 | **[Achado de UX — ainda aberto]** O formulário de edição sempre reenvia o campo `email` no payload do `PUT`, mesmo quando o admin não alterou esse campo — fazendo a API sempre entrar no ramo de verificação de duplicidade e tentativa de atualização no Firebase Auth, mesmo sem mudança real. Não é um bug funcional (a atualização para o mesmo valor é inofensiva), mas é uma chamada desnecessária ao Firebase Auth a cada "Salvar Alterações". **Nota v1.2.3:** a correção de RN-03 usa `previousEmail !== emailLower` para decidir se envia a notificação de troca — logo, esse reenvio do mesmo valor (o caso descrito aqui) **não** dispara e-mails desnecessários, mesmo entrando no ramo de verificação. | Confirmado por leitura do `handleSave` da tela de detalhe — `formData.email` sempre populado a partir dos dados carregados. |
...
| RN-09 | **[CORRIGIDO — commit `c3f18d4`]** Desde que "Suspender" passou a desabilitar de fato a conta do consultor no Firebase Auth (`disabled: true`, RN-01), o código de erro `auth/user-disabled`, lançado por `signInWithEmailAndPassword` ao tentar logar com essa conta, passou a ser efetivamente alcançável em `/login`. A partir do commit `c3f18d4`, esse código passou a ser tratado: `signIn` (`src/hooks/useAuth.ts`) agora retorna `{ success: false, error: error.code }` em vez de `error.message` (a mensagem bruta do SDK); `translateFirebaseError` (`src/app/(auth)/login/page.tsx`) ganhou um novo `if (error.includes('user-disabled')) { return 'Esta conta foi desativada. Entre em contato com o suporte.'; }`, adicionado antes do `default`. O consultor suspenso agora vê essa mensagem amigável em PT-BR, em vez do texto cru e em inglês do SDK do Firebase. Mesmo achado corrigido simultaneamente em UC-36 (RN-10), pois a causa raiz — e a correção — são compartilhadas pela mesma tela de login (`translateFirebaseError` já usa `.includes(...)`, compatível com `error.code`, então não precisou virar `switch`). **Comportamento anterior (histórico):** o consultor via a mensagem crua do SDK (`Firebase: The user account has been disabled by an administrator. (auth/user-disabled)`); achado só se tornara alcançável na prática após a correção de RN-01 (16/07/2026). | Corrigido por leitura literal do diff do commit `c3f18d4` em `src/hooks/useAuth.ts` (retorno `error.code`) e `src/app/(auth)/login/page.tsx` (`translateFirebaseError`, novo `if` para `user-disabled`). |
...
| RNF-02 | **[RESOLVIDO em v1.2.3, commit `001671b`]** O sistema agora envia confirmação por e-mail (fila `email_queue`) para o e-mail antigo e o novo quando o e-mail de login de um consultor é alterado (RN-03) — risco de suporte mitigado. | UX / Suporte |
...
- **UC-36 (Editar Usuário e Alterar Status Cross-Tenant)** — achado de UX compartilhado, **[CORRIGIDO — commit `c3f18d4`]** (RN-09 aqui / RN-10 lá): a tela de login (`login/page.tsx`) não tratava o código `auth/user-disabled` do Firebase Auth para nenhuma conta desabilitada, seja consultor (suspenso por este UC) ou usuário de clínica (status "Inativo" por UC-36) — mesma causa raiz, corrigida numa única vez (mesma tela de login serve os dois fluxos), pois `signIn` (`useAuth.ts`) passou a retornar `error.code` e `translateFirebaseError` ganhou o `case` faltante.
- **UC-04 (Fazer Login com Redirecionamento por Papel)** — dono da tela `/login` onde a correção de RN-09 foi aplicada; não documentado formalmente lá nesta rodada (correção registrada apenas em UC-29/UC-36, por ter sido descoberta e corrigida no contexto destes dois UCs).
...
- `src/app/api/consultants/[id]/route.ts` (GET/PUT/DELETE; `PUT` alterado pelo commit `001671b` — enfileira aviso de troca de e-mail em `email_queue`, RN-03)
- `src/components/auth/ProtectedRoute.tsx`
- `firestore.rules` (`consultants/{consultantId}`, função `consultantHasAccess`)
- `src/types/index.ts` (`Consultant`, `ConsultantStatus`)
- `src/hooks/useAuth.ts` (`signIn` — **[CORRIGIDO, commit `c3f18d4`]** agora retorna `error.code` em vez de `error.message`, ver RN-09)
- `src/app/(auth)/login/page.tsx` (`translateFirebaseError` — **[CORRIGIDO, commit `c3f18d4`]** ganhou tratamento para `user-disabled`, ver RN-09)
- Commits da correção: `b08fdeb` (preserva claims reais na desativação), `dbf128f` (reconecta "Suspender" à rota `DELETE`), `d167883` (restaura acesso/claims na reativação), `3ab00b6` (filtro "Suspensos" passa a incluir `'inactive'`), `001671b` (enfileira aviso de troca de e-mail, RN-03), `c3f18d4` (traduz `auth/user-disabled` na tela de login, RN-09)
...
3. ~~**[RN-03]** Falta de notificação ao consultor quando seu e-mail de login é alterado pelo admin — risco de suporte a ser avaliado. Ainda aberto.~~ **[RESOLVIDO em v1.2.3, commit `001671b`]** A API passou a enfileirar, em `email_queue`, um aviso para o e-mail antigo e para o novo sempre que a troca de fato ocorre (`previousEmail !== emailLower`).
...
6. ~~**[RN-09, achado novo]** A tela de login (`login/page.tsx`) não traduz o código `auth/user-disabled` para uma mensagem amigável em PT-BR — um consultor suspenso vê a mensagem crua e em inglês do SDK do Firebase ao tentar logar. Correção sugerida (ainda não implementada): adicionar tratamento de `user-disabled` em `translateFirebaseError`. Achado só se tornou alcançável na prática após a correção de RN-01 (16/07/2026), que passou a desabilitar a conta de verdade no Firebase Auth. Mesmo achado documentado em UC-36 (RN-10) — correção, se priorizada, deve ser feita uma única vez (mesma tela de login serve os dois fluxos).~~ **[RESOLVIDO — commit `c3f18d4`]** `signIn` (`useAuth.ts`) passou a retornar `error.code` em vez de `error.message`, e `translateFirebaseError` (`login/page.tsx`) ganhou `if (error.includes('user-disabled'))`, exibindo "Esta conta foi desativada. Entre em contato com o suporte." em vez do texto cru do SDK. Correção feita simultaneamente em UC-36 (RN-10), mesma tela de login.
7. **[RN-08, ainda aberto]** Nota v1.2.3: a correção de RN-03 (aviso de troca de e-mail) não altera a ordem de escrita assimétrica documentada em RN-08 — permanece como pendência independente.
...
| 1.2.3 | 24/07/2026 | Guilherme Scandelari | Correção pontual (commit `001671b`): RN-03 marcada como corrigida — quando o e-mail de login de um consultor é efetivamente alterado por um System Admin (`PUT /api/consultants/[id]`), o sistema agora enfileira em `email_queue` uma notificação para o e-mail antigo e para o novo, avisando da troca e orientando contato com o suporte em caso de alteração não reconhecida; envio tolerante a falha (`try/catch` que só loga). Seções 4.1, 6 (passo 7), 9 (RN-03, RN-05), 10 (RNF-02), 12, 13 e 14 atualizadas. |
| 1.2.4 | 26/07/2026 | Guilherme Scandelari | **Correção de bug (commit `c3f18d4`)**: RN-09 corrigida — `signIn` (`src/hooks/useAuth.ts`) passou a retornar `error.code` em vez de `error.message`; `translateFirebaseError` (`src/app/(auth)/login/page.tsx`) ganhou `if (error.includes('user-disabled')) { return 'Esta conta foi desativada. Entre em contato com o suporte.'; }`. Consultor suspenso agora vê mensagem amigável em PT-BR ao tentar logar, em vez do texto cru em inglês do SDK. Correção feita simultaneamente em UC-36 (RN-10), mesma tela de login. Seções 4.1b, 8 (Fluxo 8f), 9 (RN-09), 12, 13 e 14 (item 6) atualizadas. |
