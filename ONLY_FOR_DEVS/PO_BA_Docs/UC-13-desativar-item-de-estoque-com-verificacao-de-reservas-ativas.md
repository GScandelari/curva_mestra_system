...
### 2.1 Ator Primário
**Clinic Admin** — o botão "Desativar" só é exibido quando `claims.role === "clinic_admin"` (`isAdmin`). **Esta é uma restrição apenas de interface:** a regra do Firestore para subcoleções do tenant (`belongsToTenant(tenantId)`) permite leitura e escrita a qualquer usuário do tenant — incluindo `clinic_user` — não havendo *enforcement* de role a nível de dados para `deactivateInventoryItem`/`forceDeactivateInventoryItem` (ver RN-09/seção 14 — achado ampliado nesta revisão, não corrigido).
...
| RN-09 | **[Achado ampliado nesta revisão — não corrigido, decisão adiada pelo usuário]** A restrição "apenas `clinic_admin` pode desativar" é aplicada somente na interface (`isAdmin && ...`). Investigação mais aprofundada (realizada em conjunto com UC-15/RN-07) confirmou que este não é um problema isolado de `inventory` ou `stock_limits`: a regra genérica `match /tenants/{tenantId}/{document=**} { allow read, write: if belongsToTenant(tenantId); }` em `firestore.rules` já concede leitura e escrita irrestritas a **qualquer** usuário do tenant para **qualquer** subcoleção do sistema (incluindo `inventory`, `stock_limits`, `notifications`, `settings/notifications`, entre outras) — não apenas para as duas subcoleções originalmente catalogadas. Além disso, o Firestore combina múltiplos blocos `match` que casam o mesmo caminho com semântica **OR**: se qualquer bloco permitir a operação, ela é permitida, mesmo que outro bloco mais restrito também exista. Isso significa que **adicionar um novo bloco `match` mais restrito apenas para `inventory` (ex.: exigindo `role == 'clinic_admin'` para escrita) não teria nenhum efeito prático** — a regra genérica já permitiria a mesma operação de qualquer forma. Corrigir isso de verdade exigiria alterar a própria regra genérica, usada por todas as subcoleções do tenant no sistema inteiro — uma mudança de escopo muito maior do que corrigir apenas esta ação específica. O usuário decidiu **não corrigir agora**; fica registrado como achado técnico de arquitetura de segurança para decisão dedicada futura (ver seção 14). | Confirmado por leitura de `firestore.rules` (regra genérica de `tenants/{tenantId}/{document=**}` e semântica de combinação OR entre blocos `match` do Firestore) — investigação realizada durante o commit `216b3a0`, achado ampliado em relação ao originalmente descrito (mesmo achado também registrado em UC-15/RN-07). |
...
- **UC-15 (Configurar Limite de Estoque Baixo por Produto)** — RN-07 daquele UC documenta o mesmo achado ampliado de arquitetura de segurança registrado aqui em RN-09 (regra genérica do Firestore torna inefetiva qualquer regra dedicada adicional).
...
- `firestore.rules` (regra genérica de `tenants/{tenantId}/{document=**}` e semântica de combinação OR entre blocos `match` — RN-09, achado ampliado no commit `216b3a0`)
...
4. **[Achado ampliado nesta revisão — decisão adiada]** RN-09 — a restrição de role para desativar é só na interface; a investigação (commit `216b3a0`) revelou que o problema é mais sério do que inicialmente descrito: a regra genérica de subcoleção do tenant já concede escrita irrestrita a qualquer usuário do tenant para TODAS as subcoleções (não só `inventory`), e a semântica OR do Firestore entre blocos `match` torna qualquer regra específica adicional (só para `inventory`) inefetiva sem alterar a regra genérica em si — que é compartilhada por todo o sistema (incluindo `notifications`, `settings/notifications`, etc.). Corrigir isso exigiria uma mudança na regra genérica, afetando a arquitetura de segurança inteira, não apenas esta ação. O usuário decidiu não corrigir agora; requer decisão dedicada futura.
