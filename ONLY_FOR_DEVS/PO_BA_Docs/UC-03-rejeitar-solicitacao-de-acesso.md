...
4. Sistema abre um Dialog "Rejeitar Solicitação" com um campo de texto "Motivo da rejeição" (opcional). **[CORRIGIDO — commit `1254abb`]** Na tela do System Admin, o placeholder passou a ser "Ex: Perfil não compatível, dados incorretos, vaga indisponível, etc." — alinhado ao placeholder já usado na tela do Clinic Admin ("Ex: Perfil não compatível, dados incorretos, etc."), sem mais nenhuma menção a CPF/CNPJ (schema desatualizado desde UC-01, v2.0). Ambos os placeholders continuam sendo apenas exemplos ilustrativos em um campo de texto livre, sem efeito funcional.
...
~~**[Observação confirmada, sem necessidade de correção funcional]** O placeholder do campo "Motivo da rejeição" na tela `/admin/access-requests` (`"Ex: CPF/CNPJ não cadastrado, limite de usuários atingido, etc."`) é um texto de exemplo desatualizado — referencia o schema antigo de solicitação (CPF/CNPJ), que não existe mais desde a reescrita de UC-01 (v2.0). Sem efeito funcional (é apenas um exemplo ilustrativo dentro de um campo de texto livre), mas vale corrigir a cópia em uma manutenção futura de UI. Não confirmado pelo usuário como prioridade de correção.~~ **[RESOLVIDO — commit `1254abb`]** O placeholder foi atualizado para `"Ex: Perfil não compatível, dados incorretos, vaga indisponível, etc."`, alinhado ao placeholder já usado na tela equivalente do Clinic Admin, sem mais nenhuma menção a CPF/CNPJ. Sem efeito funcional (era apenas um exemplo ilustrativo em campo de texto livre) — ver Fluxo Principal, passo 4.
...
| 1.3.1 | 06/08/2026 | Guilherme Scandelari (via uml-use-case-writer) | **Correção de bug (commit `1254abb`, item UC-03-Q1)**: o placeholder desatualizado do campo "Motivo da rejeição" na tela `/admin/access-requests` (`"Ex: CPF/CNPJ não cadastrado, limite de usuários atingido, etc."`, citava um schema removido desde UC-01 v2.0) foi corrigido para `"Ex: Perfil não compatível, dados incorretos, vaga indisponível, etc."`, alinhado ao placeholder já correto usado na tela equivalente do Clinic Admin. Fluxo Principal (passo 4) e o primeiro item da seção 14 (marcado `[RESOLVIDO]`) atualizados. Correção puramente de cópia/UI, sem efeito funcional. |
