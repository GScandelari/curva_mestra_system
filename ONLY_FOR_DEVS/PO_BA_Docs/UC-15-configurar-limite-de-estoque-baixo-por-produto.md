...
| RN-07 | **[Achado ampliado nesta revisão — não corrigido, decisão adiada pelo usuário]** A restrição "só `clinic_admin` configura limites" é aplicada corretamente na renderização da aba (`isAdmin && ...`) — diferente de UC-14, aqui a UI de fato esconde a funcionalidade. Porém, investigação mais aprofundada (mesma investigação de UC-13/RN-09) confirmou que este não é um problema isolado de `stock_limits`: a regra genérica `match /tenants/{tenantId}/{document=**} { allow read, write: if belongsToTenant(tenantId); }` em `firestore.rules` já concede leitura e escrita irrestritas a qualquer usuário do tenant para **qualquer** subcoleção do sistema — e o Firestore combina múltiplos blocos `match` que casam o mesmo caminho com semântica **OR** (se qualquer bloco permitir, a operação é permitida). Logo, adicionar uma regra dedicada e mais restrita apenas para `stock_limits` **não teria nenhum efeito prático** enquanto a regra genérica continuar permitindo a mesma operação. Corrigir isso de verdade exigiria alterar a regra genérica em si, compartilhada por todas as subcoleções do tenant (incluindo `notifications`, `settings/notifications`, etc.) — mudança de escopo muito maior do que apenas esta tela. O usuário decidiu **não corrigir agora**; registrado como achado técnico de arquitetura de segurança para decisão dedicada futura (ver seção 14 e UC-13/RN-09, mesmo achado). | Confirmado pela ausência de qualquer `match /stock_limits/` em `firestore.rules`, e pela semântica OR de combinação de blocos `match` do Firestore — achado ampliado nesta revisão (investigação do commit `216b3a0`, não aplicado ao código, apenas à documentação), em conjunto com UC-13/RN-09. |
...
- `src/lib/services/alertTriggers.ts` (`checkLowStock`, `runAllChecks` — consumidor real do limite; filtro `active: true` adicionado no commit `216b3a0`, RN-08)
...
- `firestore.rules` (ausência de regra dedicada para `stock_limits` — RN-07, achado ampliado no commit `216b3a0`)

---

## 14. Perguntas em Aberto / Decisões Pendentes

1. **[Confirmado, achado não solicitado mas relevante — adiado]** RN-05 — não há nenhum agendamento automático rodando as verificações de alerta; tudo depende de um `clinic_admin` acionar manualmente a aba "Alertas". Configurar um limite aqui, isoladamente, não gera nenhum alerta por si só. Adiado — mesma dependência de infraestrutura (Cloud Scheduler) ainda inexistente que UC-42/RN-05.
...
5. **[Achado ampliado — decisão adiada pelo usuário]** RN-07 — investigação (commit `216b3a0`) revelou que o problema é mais sério do que descrito originalmente: a regra genérica de subcoleção do tenant já concede escrita irrestrita a qualquer usuário do tenant para todas as subcoleções, e a semântica OR do Firestore torna inefetiva qualquer regra dedicada adicional só para `stock_limits`, sem alterar a regra genérica compartilhada por todo o sistema. Requer decisão dedicada — não corrigido agora.
