...
> Um System Admin cadastra um novo Consultor Rennova (nome, e-mail, telefone). O sistema gera um código único de 6 dígitos e uma senha temporária, cria o usuário no Firebase Auth com `requirePasswordChange: true`, e envia um e-mail de boas-vindas — **a partir da v1.1 (commit `66c75fa`), esse e-mail não contém mais a senha em texto plano** (achado de severidade Alta corrigido; o mesmo padrão de risco havia sido identificado no UC-21, corrigido no mesmo commit). O mecanismo de troca obrigatória de senha no primeiro acesso (UC-06) já era corretamente enforced desde a v1.0, independentemente do conteúdo do e-mail. **Atualização (v1.1.1, commit `001671b`):** a ausência de rollback identificada em RN-05 foi corrigida — se qualquer etapa após `adminAuth.createUser` falhar (criação do documento `consultants`, definição de custom claims, ou criação do documento `users`), a API agora deleta o usuário recém-criado no Firebase Auth antes de relançar o erro original, evitando o usuário órfão que antes bloqueava permanentemente aquele e-mail para novas tentativas (`auth/email-already-exists`).
...
- **[CORRIGIDO em v1.1.1, commit `001671b`]** Se a criação do usuário no Auth **suceder**, mas a escrita do documento `consultants`, a definição de custom claims, ou a escrita do documento `users` falhar depois: essas três etapas agora estão envolvidas em um único `try/catch`; em caso de falha, a API chama `adminAuth.deleteUser(userId)` antes de relançar o erro original, evitando o usuário órfão que ficava para trás anteriormente (RN-05). Esse comportamento agora está alinhado ao padrão de rollback já documentado no UC-21 para o fluxo de criação de tenant+admin. Se o próprio `deleteUser` de compensação falhar, o erro é apenas logado (`console.error`) e o erro original ainda é relançado — nesse cenário extremo (dupla falha), o órfão ainda pode ocorrer, mas é um cenário residual, não o caminho principal de falha.
...
11. **[Bloco protegido por `try/catch`, v1.1.1, commit `001671b`]** API cria o documento em `consultants` (`code`, `name`, `email`, `phone`, `status: 'active'`, `authorized_tenants: []`, `created_by`).
...
13. API cria o documento espelho em `users/{uid}` com os mesmos dados e `requirePasswordChange: true`. **Se qualquer uma das etapas 11-13 falhar**, a API chama `adminAuth.deleteUser(userId)` (revertendo o usuário Auth criado no passo 10) antes de relançar o erro original — evitando o usuário órfão documentado em RN-05 (corrigido no commit `001671b`).
...
### 8e. Falha após criação do usuário no Auth (achado corrigido — rollback implementado)
...
2. **[CORRIGIDO em v1.1.1, commit `001671b`]** API captura a falha, chama `adminAuth.deleteUser(userId)` para reverter o usuário Auth criado, e só então relança o erro original — API retorna 500, mas o usuário Auth **não fica mais órfão** na maioria dos cenários (RN-05). Se `deleteUser` também falhar (cenário residual, ex.: indisponibilidade do Firebase Auth exatamente nessa segunda chamada), o erro de reversão é apenas logado (`console.error`) e o erro original ainda é relançado — nesse caso extremo específico, o órfão ainda pode ocorrer.
...
| RN-05 | **[CORRIGIDO em v1.1.1, commit `001671b`]** As etapas após a criação do usuário no Firebase Auth (escrita em `consultants`, `setCustomUserClaims`, escrita em `users`) foram envolvidas em um `try/catch`; em caso de falha, a API chama `adminAuth.deleteUser(userId)` antes de relançar o erro original — alinhando este fluxo ao padrão de rollback já documentado no UC-21 para criação de tenant+admin. Um usuário Auth "órfão" deixa de ficar para trás no caminho principal de falha. **Ressalva:** se a própria chamada de compensação (`deleteUser`) falhar, o erro é apenas logado e o erro original relançado — nesse cenário residual (dupla falha), o órfão ainda é teoricamente possível. A etapa de e-mail de boas-vindas (já tolerante a falha) permanece fora desse bloco de rollback — falha de e-mail não reverte a criação do consultor, por decisão deliberada. | Confirmado por diff do commit `001671b` em `src/app/api/consultants/route.ts` — bloco `try { ...criar consultants, claims, users... } catch (postCreateError) { await adminAuth.deleteUser(userId)...; throw postCreateError }`. |
...
- **UC-21 (Cadastrar Nova Clínica)** — mesmo padrão de risco de senha em texto plano (RN-04), corrigido no mesmo commit `66c75fa` (lá, RN-03); e mesmo padrão de rollback do usuário Auth órfão (RN-05), agora replicado aqui pelo commit `001671b`.
...
- `src/app/api/consultants/route.ts` (GET/POST; alterado pelo commit `66c75fa` — `generateConsultantWelcomeEmail` sem o parâmetro `tempPassword`, RN-04; alterado novamente pelo commit `001671b` — rollback do usuário Auth em caso de falha parcial, RN-05)
...
2. ~~**[RN-05]** Ausência de rollback em caso de falha parcial após a criação do usuário no Auth — pode deixar usuários órfãos no Firebase Auth.~~ **[RESOLVIDO em v1.1.1, commit `001671b`]** A API agora reverte (`adminAuth.deleteUser`) o usuário Auth criado se qualquer etapa seguinte falhar. Ressalva residual: se a própria chamada de reversão falhar, o órfão ainda é teoricamente possível (cenário de dupla falha, não corrigido nem endereçado nesta rodada).
...
| 1.1.1 | 24/07/2026 | Guilherme Scandelari | Correção pontual (commit `001671b`): RN-05 marcada como corrigida — as etapas após `adminAuth.createUser` (criar `consultants`, `setCustomUserClaims`, criar `users`) foram envolvidas em `try/catch`; em caso de falha, a API chama `adminAuth.deleteUser(userId)` antes de relançar o erro original, evitando o usuário órfão no Firebase Auth. Ressalva registrada: falha na própria chamada de reversão ainda pode, em cenário residual de dupla falha, deixar o órfão. Seções 4.2, 6 (passos 11-13), 8e, 9, 12, 13 e 14 atualizadas. |
