...
2. **Agora:** `NotificationBell` calcula `canDelete = claims?.role === 'clinic_admin'` e só renderiza o ícone de lixeira individual e o bloco de "Limpar notificações lidas" quando `canDelete` é verdadeiro (RN-02) — um `clinic_user` deixa de ver esses controles, então este cenário não ocorre mais pela UI padrão. Se a mesma chamada fosse feita fora da UI (ex.: manipulando o client diretamente), a regra do Firestore continuaria rejeitando... **exceto que, na prática, ela não rejeita** — ver ressalva adicionada em RN-02 (seção 9): a regra genérica de subcoleção do tenant já permite a operação a qualquer usuário do tenant via semântica OR, tornando a regra dedicada de `delete` inefetiva.
...
| RN-02 | **[CORRIGIDO no commit `eb12c91` — UC-44-RN-02]** Antes: a regra do Firestore (`firestore.rules`, linhas 65-74) restringia `delete` em `tenants/{tenantId}/notifications/{notificationId}` a `role == 'clinic_admin'`, mas a UI (`NotificationBell`) exibia o botão de exclusão individual e "Limpar notificações lidas" identicamente para `clinic_admin` e `clinic_user` (nenhum gate de role no componente) — um `clinic_user` que tentasse excluir sofria uma falha de permissão silenciosa (ver 8b, histórico). Agora: `NotificationBell` declara `const canDelete = claims?.role === 'clinic_admin'` e envolve o botão de exclusão individual (dentro do `map` de notificações) e o bloco inteiro do botão "Limpar notificações lidas" em `{canDelete && (...)}` — ambos deixam de ser renderizados para `clinic_user`, alinhando a UI à regra do Firestore que já restringia a ação a esse role. **[Ressalva adicionada em 25/07/2026]**: essa regra dedicada, embora presente no arquivo, é hoje inefetiva na prática — a regra genérica de subcoleção do tenant (`tenants/{tenantId}/{document=**}`) já concede a mesma operação (`delete`) a qualquer usuário do tenant via semântica OR do Firestore (múltiplas regras que casam o mesmo caminho são combinadas com OR, não com "mais específica vence"). Ou seja: um `clinic_user` poderia, tecnicamente, excluir uma notificação chamando a API do Firestore diretamente, fora da UI que só oculta os botões (`canDelete`) — a proteção real hoje é só o gate de UI, não uma regra de segurança de verdade, mesmo isso já sendo tecnicamente verdade desde antes desta correção. Ver achado de severidade Alta consolidado no mapa de bugs (UC-13-RN-09/UC-15-RN-07) para o contexto completo e a decisão de adiar a correção. | Correção confirmada por leitura do commit `eb12c91` (`NotificationBell.tsx`) — `canDelete` declarado a partir de `claims?.role`, usado para condicionar a renderização dos dois blocos de exclusão; comparado a `firestore.rules` (linhas 65-74, `allow create, delete: if belongsToTenant(tenantId) && request.auth.token.role == 'clinic_admin'`), inalterada nesta correção. Ressalva confirmada por releitura de `firestore.rules` linhas 58-67 (regra genérica `match /tenants/{tenantId}/{document=**}`, `allow read, write: if belongsToTenant(tenantId)`, sem checagem de `role`). |
...
- `firestore.rules` (linhas 65-74 — regra dedicada de `tenants/{tenantId}/notifications`, inalterada nesta correção; linhas 58-67 — regra genérica de subcoleção do tenant que torna a regra dedicada de `delete` inefetiva na prática, ver ressalva de RN-02)
...
6. **[Ressalva de precisão factual adicionada em 25/07/2026 — não é decisão nova]** A regra dedicada de RN-02 (`delete` restrito a `clinic_admin`) é hoje inefetiva na prática, pois a regra genérica de subcoleção do tenant já permite a mesma operação a qualquer usuário do tenant via semântica OR do Firestore. Este achado foi registrado com severidade Alta no mapa de bugs (`_MAPA-DE-BUGS-E-MELHORIAS.md`, UC-13-RN-09/UC-15-RN-07), com correção adiada por decisão do usuário — corrigir exigiria alterar a regra genérica usada por todo o sistema multi-tenant, não apenas esta subcoleção.
