...
5. Sistema valida no frontend: nova senha aprovada por `validatePassword(newPassword, { minLength: 6 })` — função compartilhada de `@/lib/validations/serverValidations` (**[CORRIGIDO, commit `1254abb`]**, ver RN-02; antes, uma função local inline só checava `password.length < 6`); nova senha igual à confirmação; nova senha diferente da senha atual.
...
1. Nova senha reprovada por `validatePassword` (menos de 6 caracteres, ou sem nenhuma letra — **[CORRIGIDO, commit `1254abb`]**, esta segunda checagem é nova em relação à função local anterior, ver RN-02), nova senha diferente da confirmação, ou nova senha igual à senha atual.
2. Sistema exibe a mensagem específica: "Senha deve ter pelo menos 6 caracteres" ou "Senha deve conter pelo menos uma letra" (retornadas por `validatePassword`), "As senhas não coincidem", ou "A nova senha deve ser diferente da senha atual".
...
| RN-02 | **[CORRIGIDO — commit `1254abb`]** Antes, a validação de força da nova senha nesta tela usava uma função local inline que só checava `password.length < 6` — mais simples que a `validatePassword` compartilhada de `@/lib/validations/serverValidations` (que também exige pelo menos uma letra, além do tamanho mínimo). Agora, a função local foi removida e a tela chama diretamente `validatePassword(newPassword, { minLength: 6 })`, exibindo `passwordValidation.error` quando `!passwordValidation.valid`. **O mesmo achado gêmeo foi identificado e corrigido, no mesmo commit, em `src/app/(auth)/reset-password/[token]/page.tsx`** — tela coberta formalmente por **UC-08 (System Admin Envia Link de Redefinição de Senha)**, não por este UC-06; a mesmíssima função local fraca existia lá, e também foi substituída pela chamada a `validatePassword`. | Elimina a divergência de rigor entre esta tela e a validação compartilhada usada em outros pontos do sistema (ex.: UC-01, RN-04 a RN-06) — corrigido por leitura direta do diff do commit `1254abb` em `change-password/page.tsx` e `reset-password/[token]/page.tsx`. |
...
- **UC-08 (System Admin Envia Link de Redefinição de Senha)** é o mecanismo irmão: mesma origem (System Admin, na mesma tela `admin/users`/`admin/consultants`), mas usando um link por e-mail com token customizado em vez de definir a senha diretamente. Os dois convergem no ponto em que UC-08, ao ser concluído pelo usuário-alvo, também remove `requirePasswordChange` se estiver presente (ver UC-08, RN-06). **[Cross-reference, commit `1254abb`]** Os dois também convergem no achado gêmeo de validação de senha corrigido neste commit: a tela de UC-08 (`reset-password/[token]/page.tsx`) tinha a mesma função local fraca de validação de senha desta tela, substituída pela mesma `validatePassword` compartilhada (ver RN-02).
...
- `src/lib/validations/serverValidations.ts` (`validatePassword`, usada por esta tela desde o commit `1254abb` — ver RN-02)
...
| 1.6 | 06/08/2026 | Guilherme Scandelari (via uml-use-case-writer) | **Correção de bug (commit `1254abb`, item UC-06-RN-02)**: a validação fraca de nova senha desta tela (função local inline, só checava tamanho mínimo) foi substituída pela função compartilhada `validatePassword` de `@/lib/validations/serverValidations` (`validatePassword(newPassword, { minLength: 6 })`), que também exige pelo menos uma letra. O mesmo achado gêmeo foi corrigido, no mesmo commit, em `src/app/(auth)/reset-password/[token]/page.tsx` — tela coberta por **UC-08**, não por este UC-06; a correção lá não foi documentada neste UC além desta referência cruzada. RN-02 marcada `[CORRIGIDO]`; atualizados Fluxo Principal (passo 5), Fluxo de Exceção 8d (novas mensagens possíveis), Referências (seção 13) e Casos de Uso Relacionados (seção 12, bullet de UC-08). |
