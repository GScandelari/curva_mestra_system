...
3. Caso contrário, sistema redireciona diretamente por role (mesma lógica do passo 9), **sem repetir** as checagens de `role`/`active` (passo 6) nem a de clínica suspensa (passo 8) — este caminho específico (`/login`, usuário já autenticado) não foi alterado pela correção de `ProtectedRoute.tsx` (RN-09); permanece como comportamento as-is, sem gap de segurança relevante já que a checagem de `requirePasswordChange` já é feita aqui. **Ver RN-11:** é exatamente este mesmo `useEffect` (linhas 42-59 de `src/app/(auth)/login/page.tsx`) que, ao correr em paralelo com o fluxo manual de `handleSubmit` durante uma submissão de login (não apenas no acesso direto a `/login` descrito aqui), origina a condição de corrida documentada no Fluxo de Exceção 8d.
...
2. Sistema redireciona para `/waiting-approval` — tela reaproveitada tanto para quem aguarda a primeira aprovação (pós-UC-01) quanto para quem foi desativado posteriormente por um admin. **[CORRIGIDO — commit `1254abb`, UC-04-Q2]** Desde essa correção, a própria tela `/waiting-approval` diferencia os dois cenários pela mensagem exibida ao usuário — ver RN-10 e seção 14 (comportamento anterior, antes uniforme para os dois casos, documentado ali como histórico).
...
> **[Achado confirmado — `UC-04-Q4`, ver RN-11]** Os passos 1-4 acima descrevem o comportamento *pretendido* deste fluxo, tal como implementado na lógica sequencial de `handleSubmit`/`checkClinicStatus`/`handleInactiveClinic`. **Na prática, porém, existe uma condição de corrida real e confirmada empiricamente** entre essa lógica sequencial e o `useEffect` reativo "Redirecionar se já estiver autenticado" (`src/app/(auth)/login/page.tsx`, linhas 42-59), que compete com ela pelo redirecionamento pós-login. Se o `useEffect` reativo disparar antes da checagem manual de `tenant.active` (passo 1 acima) terminar, o `clinic_user` é levado brevemente a `/clinic/dashboard`; o `signOut()` do fluxo manual, que continua rodando em paralelo, força o retorno a `/login` logo em seguida — mas essa nova navegação remonta o componente da página de login do zero, cujo estado local `clinicInactiveMessage` nasce em `false` novamente. **O card "Sistema Indisponível" do passo 2 nunca chega a ser exibido** nesse cenário: o `clinic_user` de uma clínica suspensa vê apenas o formulário de login vazio/resetado, sem nenhuma explicação do que aconteceu. Ver RN-11 para o mecanismo técnico completo e a Seção 14 para o registro formal desta pendência conhecida e deliberadamente não corrigida.
...
| RN-03 | A verificação de clínica ativa (passo 8) só ocorre para usuários com `tenant_id` definido e que não sejam `system_admin` — isso exclui tanto `system_admin` (sem `tenant_id`) quanto, na prática, `clinic_consultant` (cujo `tenant_id` costuma ser `null`, atuando sobre múltiplos tenants via `authorized_tenants`). **[Ver RN-11]** Esta checagem é executada de forma assíncrona e sequencial dentro de `handleSubmit`, o que a torna suscetível à condição de corrida descrita naquela regra quando compete com o `useEffect` reativo de redirecionamento. | System Admin e consultores operam fora do contexto de uma única clínica. |
| RN-04 | O tratamento de clínica inativa é diferenciado por role: `clinic_user` é desconectado e bloqueado com mensagem informativa (sem acesso a nenhuma tela do sistema); `clinic_admin` é redirecionado para `/clinic/my-clinic`, mantendo acesso a uma tela específica. **[Ver RN-11]** Para `clinic_user`, a exibição efetiva dessa mensagem informativa está sujeita à condição de corrida descrita naquela regra — o bloqueio de acesso (via `signOut()`) ocorre de forma confiável, mas a mensagem explicativa ao usuário pode não chegar a ser exibida. | Permite que o responsável pela clínica (`clinic_admin`) tenha visibilidade/ação sobre a própria suspensão, enquanto usuários comuns ficam bloqueados. |
...
| RN-10 | **[CORRIGIDO — commit `1254abb`, UC-04-Q2]** Antes: a tela `/waiting-approval` (destino do Fluxo de Exceção 8b) exibia sempre a mesma mensagem ("Conta Criada com Sucesso" / "Aguardando aprovação do administrador" / "Sua conta foi criada com sucesso. No entanto, você precisará aguardar que um administrador do sistema configure seu acesso...") tanto para quem nunca teve `role` atribuído (primeiro acesso, aguardando aprovação pós-UC-01) quanto para quem já foi aprovado antes e depois desativado por um admin (`claims.active === false`). Agora: a tela usa `useAuth()` (desestruturando também `claims` e `loading`) para calcular `isDeactivated = !loading && claims !== null` — `claims` só é não-nulo quando o usuário já teve um `role` atribuído em algum momento (ver `extractCustomClaims` em `useAuth.ts`, que retorna `null` se `!claims.role`); o guard `!loading` evita mostrar a mensagem errada durante a janela de carregamento do token do Firebase. Quando `isDeactivated` é `true`, o título muda para "Conta Desativada", a descrição para "Acesso suspenso pelo administrador" e o texto explicativo para "Sua conta foi desativada por um administrador do sistema." | Elimina a mensagem enganosa exibida a um usuário desativado, que antes via um texto de "conta recém-criada, aguardando aprovação" mesmo já tendo tido acesso normal ao sistema anteriormente. Confirmado por leitura direta de `src/app/(auth)/waiting-approval/page.tsx`. |
| RN-11 | **[NÃO CORRIGIDO — decisão deliberada do usuário, achado `UC-04-Q4`]** No Fluxo de Exceção 8d (clínica inativa/suspensa, role `clinic_user`), existe uma condição de corrida real e confirmada empiricamente entre duas lógicas de redirecionamento pós-login que competem entre si em `src/app/(auth)/login/page.tsx`: (1) o `useEffect` reativo "Redirecionar se já estiver autenticado" (linhas 42-59), que dispara assim que `isAuthenticated && claims` ficam disponíveis via `onAuthStateChanged` (dentro de `useAuth`) e redireciona imediatamente por role — inclusive `clinic_user` → `/clinic/dashboard` — sem esperar a checagem de clínica ativa; e (2) a checagem manual sequencial dentro de `handleSubmit` (linha 99 em diante), que só chega a `checkClinicStatus(claims)` (linha 126, após os `await`s de `getIdToken(true)` e `getIdTokenResult()`) e, ao confirmar `tenant.active === false`, chama `handleInactiveClinic('clinic_user')` (linhas 61-67), que executa `signOut()` e só depois `setClinicInactiveMessage(true)`. **Se o `useEffect` reativo (1) disparar antes da checagem manual (2) terminar**, o `clinic_user` é levado brevemente a `/clinic/dashboard`; o `signOut()` de (2), que continua rodando em paralelo, força o retorno a `/login` logo em seguida — mas essa nova navegação remonta o componente da página de login do zero, cujo estado local `clinicInactiveMessage` nasce em `false` novamente. **Resultado:** o card "Sistema Indisponível" descrito no passo 2 do Fluxo de Exceção 8d **nunca chega a ser exibido** — o `clinic_user` de uma clínica suspensa vê apenas o formulário de login vazio/resetado, sem nenhuma explicação do que aconteceu. É uma falha de UX de segurança (usuário desconectado sem explicação alguma), confirmada empiricamente via screenshot do Playwright contra o Firebase Emulator Suite real (não é suposição) durante a geração do caderno de teste retroativo deste UC pelo `qa-agent`. | Documenta um comportamento **as-is real e confirmado**, não uma hipótese: descoberto e validado empiricamente pelo `qa-agent` (screenshot de falha do Playwright), registrado como `UC-04-Q4` (Alta, Aberto) em `_MAPA-DE-BUGS-E-MELHORIAS.md` (Seção 2). **Decisão do usuário: não corrigir nesta rodada** — fluxo de autenticação é sensível, correção tratada como task separada e deliberadamente adiada, não pendente por falta de decisão. O teste E2E correspondente (`tests/e2e/UC-04-fazer-login-com-redirecionamento-por-papel.spec.ts`, describe "Fluxo de Exceção 8d — clínica inativa/suspensa, role clinic_user (RN-03, RN-04)") está marcado com `test.fixme()` e um comentário explicativo detalhado, justamente para não bloquear o gate de CI obrigatório (CLAUDE.md, item 8) enquanto o achado não é resolvido — ver Seção 14. |
...
## 13. Referências
- `src/app/(auth)/login/page.tsx` (`redirectByRole`, linhas 87-97 — ver RN-08; `useEffect` reativo de redirecionamento, linhas 42-59, e `handleSubmit`/`checkClinicStatus`/`handleInactiveClinic`, linhas 61-135 — ver RN-11)
- `src/hooks/useAuth.ts`
- `src/app/(auth)/change-password/page.tsx`
- `src/app/(auth)/waiting-approval/page.tsx` (mensagem diferenciada entre "aguardando aprovação" e "conta desativada", desde o commit `1254abb` — ver RN-10)
...
- `tests/e2e/UC-04-fazer-login-com-redirecionamento-por-papel.spec.ts` (describe "Fluxo de Exceção 8d — clínica inativa/suspensa, role clinic_user (RN-03, RN-04)", marcado com `test.fixme()` — teste correspondente ao achado `UC-04-Q4`, ver RN-11)
...
- Commit da correção: `1254abb` (`fix: seis itens de baixa severidade (UC-01, UC-03, UC-04, UC-06, UC-07)`) — diferencia a mensagem de `/waiting-approval` entre "aguardando aprovação" e "conta desativada" (RN-10)
- `_MAPA-DE-BUGS-E-MELHORIAS.md` (Seção 2, item `UC-04-Q4`, Alta, Aberto) — registro formal do achado documentado nesta versão como RN-11

---

## 14. Perguntas em Aberto / Decisões Pendentes

**[Achado confirmado, deliberadamente NÃO corrigido nesta rodada — `UC-04-Q4`]** Existe uma condição de corrida real e confirmada empiricamente entre o `useEffect` reativo de redirecionamento pós-autenticação (linhas 42-59 de `src/app/(auth)/login/page.tsx`) e a checagem manual sequencial de `tenant.active` dentro de `handleSubmit`/`checkClinicStatus` (linha 126) — ver RN-11 e Fluxo de Exceção 8d para o mecanismo completo. Se o efeito reativo disparar primeiro, o `clinic_user` de uma clínica suspensa é levado brevemente a `/clinic/dashboard` e depois deslogado pelo `signOut()` do fluxo manual, mas o card "Sistema Indisponível" nunca chega a ser exibido, pois a remontagem da página de login zera o estado local `clinicInactiveMessage`. **Isto não é uma lacuna esquecida de investigação:** foi descoberto e validado empiricamente (screenshot do Playwright contra o Firebase Emulator Suite real) pelo `qa-agent` durante a geração do caderno de teste retroativo deste UC, registrado como `UC-04-Q4` (Alta, Aberto) em `_MAPA-DE-BUGS-E-MELHORIAS.md` (Seção 2), e o usuário decidiu explicitamente **não corrigir nesta rodada** — o fluxo de autenticação é sensível e a correção será tratada como uma task separada, deliberadamente adiada, não pendente por falta de decisão de produto. O teste E2E correspondente (`tests/e2e/UC-04-fazer-login-com-redirecionamento-por-papel.spec.ts`, describe "Fluxo de Exceção 8d — clínica inativa/suspensa, role clinic_user (RN-03, RN-04)") está marcado com `test.fixme()` e um comentário explicativo detalhado, justamente para não bloquear o gate de CI obrigatório (ver CLAUDE.md, item 8) enquanto o achado não é resolvido.
...
~~**[As-is, sem proposta de correção]** A tela `/waiting-approval` é reaproveitada tanto para o cenário de "aguardando primeira aprovação" (pós-UC-01) quanto para "conta desativada posteriormente por um admin" — ambos exibem a mesma mensagem, que não reflete literalmente a segunda situação. Documentado como comportamento atual (Fluxo de Exceção 8b).~~ **[RESOLVIDO — commit `1254abb`, UC-04-Q2]** A tela `/waiting-approval` passou a diferenciar os dois cenários: usa `useAuth()` (`claims`, `loading`) para calcular `isDeactivated = !loading && claims !== null` e, quando verdadeiro, exibe "Conta Desativada" / "Acesso suspenso pelo administrador" / "Sua conta foi desativada por um administrador do sistema." em vez da mensagem de "conta recém-criada". Ver RN-10 e Fluxo de Exceção 8b.
...
| 1.1.5 | 06/08/2026 | Guilherme Scandelari (via uml-use-case-writer) | **Correção de bug (commit `1254abb`, item UC-04-Q2)**: a tela `/waiting-approval` (destino do Fluxo de Exceção 8b) passou a diferenciar, pela mensagem exibida, o cenário de "aguardando primeira aprovação" do de "conta desativada posteriormente por um admin" — antes ambos exibiam o mesmo texto. A tela agora usa `useAuth()` (`claims`, `loading`) para calcular `isDeactivated = !loading && claims !== null`, exibindo "Conta Desativada" / "Acesso suspenso pelo administrador" quando o usuário já teve um `role` atribuído em algum momento. Adicionada RN-10 (Seção 9, marcada `[CORRIGIDO]`); atualizados Fluxo de Exceção 8b, Referências (Seção 13) e o segundo item da Seção 14 (marcado `[RESOLVIDO]`). |
| 1.2 | 26/08/2026 | Guilherme Scandelari (via uml-use-case-writer) | **Achado `UC-04-Q4` incorporado ao UC (Alta, Aberto, `_MAPA-DE-BUGS-E-MELHORIAS.md`, Seção 2)**: descoberto pelo `qa-agent` ao gerar o caderno de teste retroativo deste UC (Playwright + Firebase Emulator Suite) e validado empiricamente via screenshot de falha — existe uma condição de corrida real em `src/app/(auth)/login/page.tsx` entre o `useEffect` reativo de redirecionamento pós-autenticação (linhas 42-59) e a checagem manual sequencial de `tenant.active` em `handleSubmit`/`checkClinicStatus`/`handleInactiveClinic` (linhas 61-135), que pode impedir a exibição do card "Sistema Indisponível" ao `clinic_user` de uma clínica suspensa (Fluxo de Exceção 8d). Adicionada RN-11 (Seção 9, documentando o comportamento as-is, explicitamente **não corrigido**); adicionada ressalva textual em RN-03 e RN-04 remetendo à RN-11; atualizado o Fluxo de Exceção 8d com nota detalhada sobre o achado; atualizado o Fluxo Alternativo 7a com referência cruzada; adicionada Referência (Seção 13) ao spec `tests/e2e/UC-04-fazer-login-com-redirecionamento-por-papel.spec.ts` e ao próprio item do mapa de bugs; adicionado item explícito e não tachado na Seção 14 registrando que este achado é **conhecido e deliberadamente não corrigido** por decisão do usuário nesta rodada — não uma lacuna de investigação esquecida. O teste E2E correspondente está marcado com `test.fixme()` para não bloquear o gate de CI obrigatório enquanto o achado não é resolvido. Nenhuma alteração de código foi feita neste UC nem no mapa de bugs — escopo desta atualização é exclusivamente o documento UC-04. |
