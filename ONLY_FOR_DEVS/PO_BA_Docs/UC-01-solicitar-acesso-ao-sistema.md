...
- `src/lib/firebase-admin.ts` (`adminDb`, usado desde o commit `1254abb` na checagem de duplicidade de solicitação pendente, RN-09)
...
**[Achado não verificado, precisa ser testado no emulador antes de decidir severidade real]** Durante a investigação da correção de RN-09 (commit `1254abb`), foi constatado que `POST /api/access-requests` (linha ~50, já antes das mudanças deste commit) faz uma leitura `getDoc(doc(db, 'system_settings', 'global'))` usando o **client SDK** do Firebase — nesta mesma rota pública/não-autenticada. A regra do Firestore para `system_settings` exige `isAuthenticated()` (`request.auth != null && request.auth.token.active == true`), o que sugere que essa leitura pode estar sendo bloqueada por permissão para qualquer visitante não autenticado — o que quebraria o fluxo inteiro de criação de solicitação de acesso (a função inteira cairia no `catch` e retornaria 500). Essa checagem foi adicionada em um commit anterior desta mesma sessão de trabalho (`66c75fa`, batch de UC-21/UC-28/UC-35, Alta severidade) para implementar o gate de `registration_enabled` (ver RN-10). **Isso NÃO foi verificado em runtime/emulador** — é um achado por leitura de código, não confirmado. Não presumir severidade Alta/Crítica nem Baixa; fica registrado aqui para investigação e teste dedicados, a ser incorporado pelo uc-issues-tracker como um item novo e separado.
