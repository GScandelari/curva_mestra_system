...
1. ~~**[UC-36-RN-02 / UC-36-RN-03 — Crítica]** Editar "Função" ou "Status" de um usuário de clínica no Portal Admin (`admin/users`) **não tem nenhum efeito real de acesso** — não sincroniza custom claims nem desabilita a conta no Firebase Auth. Um System Admin pode acreditar que rebaixou ou desativou um usuário e nada muda de fato.~~ **[EM CORREÇÃO 15/07/2026]** Decisão de produto: construir `PUT /api/users/[id]` sincronizando claims + desabilitando Auth, com checagem de "último clinic_admin" — ver Seção 6.
...
| UC-36-RN-02 | [UC-36](UC-36-editar-usuario-e-alterar-status-cross-tenant.md) | Achado de segurança | Crítica | Editar "Função" de um usuário de clínica não sincroniza custom claims — mudança puramente cosmética no Firestore, sem efeito real de permissões. | Em Correção (nova rota `PUT /api/users/[id]`) | Admin — Usuários |
| UC-36-RN-03 | [UC-36](UC-36-editar-usuario-e-alterar-status-cross-tenant.md) | Achado de segurança | Crítica | Definir "Status: Inativo" não desabilita a conta no Firebase Auth nem existe rota alternativa correta — usuário "desativado" continua acessando normalmente. | Em Correção (nova rota `PUT /api/users/[id]`) | Admin — Usuários |
...
| ID | UC de Origem | Categoria | Severidade | Resumo | Status |
|---|---|---|---|---|---|
| UC-36-RN-07 | [UC-36](UC-36-editar-usuario-e-alterar-status-cross-tenant.md) | Débito técnico | Média | Nenhuma validação impede que uma clínica fique sem nenhum `clinic_admin` ativo após uma edição de função/status. | Em Correção (empacotado com UC-36-RN-02/03) |
...
| UC-36-RN-02, UC-36-RN-03, UC-36-RN-07 | [UC-36](UC-36-editar-usuario-e-alterar-status-cross-tenant.md) | Em Correção | 15/07/2026 | Decisão do PO: opção (a) da seção 14 do UC-36 — construir `PUT /api/users/[id]` (rota nova, não existe hoje) que chame `adminAuth.setCustomUserClaims` (sincroniza "Função") e `adminAuth.updateUser({disabled: true})` (torna "Status: Inativo" real), migrando `admin/users` de `updateDoc` direto para essa rota. Empacotado junto (RN-07): antes de rebaixar ou desativar um `clinic_admin`, a rota deve checar se ele é o único `clinic_admin` ativo do tenant; se for, exigir confirmação explícita do System Admin antes de prosseguir (não um bloqueio automático — o admin pode confirmar que quer mesmo deixar a clínica sem admin local). |

🔔 **Ação de código pendente (não implementada nesta sessão):** criar `PUT /api/users/[id]` (Admin SDK) com a lógica acima; migrar `handleSaveUser` (`admin/users/page.tsx`) de `updateDoc` para chamar essa rota. Após implementar, `uml-use-case-writer` deve atualizar UC-36 (RN-02/RN-03/RN-07 deixam de ser achados; novo fluxo de exceção documentando a confirmação de "último admin") e `uc-issues-tracker` deve mover estes itens para "Corrigido e Documentado".
