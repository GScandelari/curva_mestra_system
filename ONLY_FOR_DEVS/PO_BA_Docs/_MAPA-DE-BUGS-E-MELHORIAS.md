...
2. ~~**[UC-29-RN-01 — Crítica]** "Suspender" um Consultor Rennova (`admin/consultants`) é puramente cosmético — não bloqueia login nem revoga acesso às clínicas vinculadas.~~ **[EM CORREÇÃO 15/07/2026]** Decisão de produto: reconectar o botão "Suspender" à rota `DELETE` já existente — ver Seção 6.
...
5. ~~**[UC-34-RN-03 — Crítica]** Exclusão de Documento Legal é definitiva (hard delete) e não verifica se o documento já foi aceito por usuários — compromete a trilha de auditoria de compliance.~~ **[EM CORREÇÃO 15/07/2026]** Decisão de produto: bloquear a exclusão quando houver aceites registrados — ver Seção 6.
...
| UC-29-RN-01 | [UC-29](UC-29-editar-suspender-e-reativar-consultor.md) | Achado de segurança | Crítica | "Suspender" um consultor não bloqueia login nem revoga acesso às clínicas vinculadas — é puramente cosmético no Firestore, sem tocar Auth/claims. | Em Correção (reconectar rota `DELETE`) | Admin — Consultores |
| UC-34-RN-03 | [UC-34](UC-34-editar-publicar-e-excluir-documento-legal.md) | Débito técnico / Compliance | Crítica | Exclusão definitiva (hard delete) de documento legal sem checar `user_document_acceptances` — compromete a trilha de auditoria de compliance. | Em Correção (bloquear exclusão se houver aceites) | Admin — Documentos Legais |
...
| UC-29-RN-01, UC-29-RN-02 | [UC-29](UC-29-editar-suspender-e-reativar-consultor.md) | Em Correção | 15/07/2026 | Decisão do PO: o botão "Suspender" (`admin/consultants`) passa a chamar `DELETE /api/consultants/[id]` (já implementada, hoje órfã) em vez de `PUT` com `{status: 'suspended'}` — isso desabilita a conta no Firebase Auth e zera o custom claim `active`, corrigindo o comportamento cosmético atual. Não foi escolhido reforçar o `PUT` (opção b) para não duplicar lógica de desativação em duas rotas. |

🔔 **Ação de código pendente (não implementada nesta sessão):** trocar a chamada do botão "Suspender" de `PUT` para `DELETE` em `admin/consultants` (listagem e/ou tela de detalhe). Após implementar, `uml-use-case-writer` deve atualizar UC-29 (RN-01/RN-02 deixam de ser achados, o fluxo de suspensão passa a usar `DELETE`) e `uc-issues-tracker` deve mover estes itens para "Corrigido e Documentado".

| UC-34-RN-03 | [UC-34](UC-34-editar-publicar-e-excluir-documento-legal.md) | Em Correção | 15/07/2026 | Decisão do PO: opção (a) da seção 14 do UC-34 — bloquear a exclusão de um documento legal (`handleDelete`, `admin/legal-documents`) sempre que existir pelo menos um registro em `user_document_acceptances` referenciando aquele `document_id`, com mensagem clara ao admin. Não escolhida a opção (b) (permitir só desativar, nunca excluir) para preservar a possibilidade de excluir documentos que nunca foram aceitos por ninguém (ex.: rascunhos, erros de cadastro). |

🔔 **Ação de código pendente (não implementada nesta sessão):** em `handleDelete` (`admin/legal-documents/page.tsx`), consultar `user_document_acceptances` por `document_id` antes do `deleteDoc`; se houver qualquer resultado, bloquear a exclusão com uma mensagem explicando o motivo. Após implementar, `uml-use-case-writer` deve atualizar UC-34 (RN-03 deixa de ser achado, novo fluxo de exceção documentando o bloqueio) e `uc-issues-tracker` deve mover este item para "Corrigido e Documentado".
