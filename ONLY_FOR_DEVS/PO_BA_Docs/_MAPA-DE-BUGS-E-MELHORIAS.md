...
| UC-43-RN-01/02 | [UC-43](UC-43-configurar-preferencias-de-notificacao.md) | Bug funcional | Crítica | `initializeNotificationSettings` e o fallback de salvamento usam `updateDoc` em vez de `setDoc` — a tela pode estar permanentemente quebrada para qualquer tenant cujo documento de configurações nunca foi criado. | Em Correção (`updateDoc` → `setDoc`) | Notificações e Alertas |
...
🔔 **Ação de código pendente (não implementada nesta sessão):** trocar `updateDoc` por `setDoc` nos dois pontos de `notificationService.ts` (UC-43). Após o bug fix, `uml-use-case-writer` deve atualizar UC-43 (RN-01/02 deixam de ser achado) e `uc-issues-tracker` deve mover UC-43-RN-01/02 para "Corrigido e Documentado".
