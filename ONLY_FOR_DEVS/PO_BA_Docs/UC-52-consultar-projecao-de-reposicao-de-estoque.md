...
- Para o card no Dashboard exibir uma contagem "próxima", pelo menos um produto precisa ter dados de consumo suficientes (RN-03) dentro de uma das três janelas avaliadas (30/60/90 dias); caso contrário, o card exibe o estado vazio (Fluxo Alternativo 7b).
...
2. Ao carregar a tela, o sistema calcula, em paralelo e por `codigo_produto` (agregando todos os lotes ativos, mesma convenção de UC-15):
   ...
   b. a taxa de consumo diária, a partir de `tenants/{tenantId}/solicitacoes` com `status == 'concluida'`, usando a janela de histórico mais recente que tiver dados suficientes, tentando primeiro a janela mais estreita (mais recente) e ampliando a janela apenas quando os dados recentes forem insuficientes (cascata 30 → 60 → 90 dias — RN-03);
...
6. Sistema exibe uma tabela com todos os produtos ativos do tenant: código, nome, quantidade total disponível, taxa de consumo diária, data estimada de esgotamento (ordenada da mais próxima para a mais distante) e um aviso fixo indicando a janela de histórico usada em cada linha (ex.: "Estimativa baseada nos últimos 30 dias de consumo").
...
### 7a. Produto com dados insuficientes mesmo na janela mais longa (a partir do passo 2b/6)
1. Nenhuma das três janelas (30/60/90 dias) atinge o critério mínimo de dados suficientes (RN-03) para aquele `codigo_produto`.
...
| RN-03 | **[Proposto pelo uml-use-case-writer — ver Seção 14; ordem da cascata corrigida em v1.1 — ver Seção 15]** Critério de "dados suficientes" para uma janela ser considerada válida: existirem pelo menos **2 `Solicitacao` distintas com `status == 'concluida'`**, envolvendo o mesmo `produto_codigo`, com `dt_procedimento` em **datas de calendário diferentes**, dentro da janela avaliada. Cascata: tenta **30 dias primeiro** (janela mais estreita/recente); se não satisfizer o critério, tenta 60; se não satisfizer, tenta 90; se mesmo em 90 dias o critério não for satisfeito, o produto entra no Fluxo Alternativo 7a ("dados insuficientes"). A cascata começa pela janela mais estreita porque todas as janelas são retroativas (terminam em "hoje"): a janela de 60 dias é sempre um subconjunto da de 90, e a de 30 é sempre um subconjunto da de 60. Por isso, avaliar da mais larga para a mais estreita (90 → 60 → 30, como na v1.0 original) tornava o fallback para as janelas menores matematicamente inalcançável — se 90 dias não tivesse dados suficientes, nenhum subconjunto seu (60 ou 30 dias) poderia ter mais datas distintas, logo nunca seria "mais suficiente" que 90. A ordem 30 → 60 → 90 corrige isso: só amplia a janela quando os dados mais recentes forem de fato insuficientes. | Decisão do usuário (item 5): "pelo menos 2 eventos de consumo em datas diferentes" foi o critério sugerido no repasse como exemplo objetivo; adotado como proposta a validar tecnicamente antes da implementação (não bloqueia a aprovação deste UC) — critério em si (2 datas distintas) mantido sem alteração. Ordem da cascata corrigida pelo usuário durante a implementação (22/09/2026), ao constatar o problema de aninhamento das janelas descrito acima. |
| RN-04 | Toda vez que uma data estimada é exibida (card do Dashboard ou tela "Projeções"), o sistema deve indicar de forma visível qual janela foi efetivamente usada (30, 60 ou 90 dias) e que se trata de uma estimativa baseada em histórico — nunca apresentada como um fato garantido. | Decisão explícita do usuário (item 5), para evitar que uma projeção pouco confiável (calculada com poucos dados) seja lida como uma certeza pelo ator. |
...
- `src/lib/services/projectionService.ts` (`HISTORY_WINDOWS_DAYS = [30, 60, 90]`, `selectConsumptionWindow` — implementação da cascata corrigida, v1.1)
- `src/__tests__/projectionService.test.ts` (testes unitários de `selectConsumptionWindow` cobrindo a cascata 30 → 60 → 90, base empírica que revelou o problema de aninhamento corrigido em v1.1)
...
1. **[Proposta técnica a validar, não bloqueante]** RN-03 — o critério de "dados suficientes" (pelo menos 2 `Solicitacao concluida` em datas diferentes, por janela) foi proposto pelo `uml-use-case-writer` a partir do exemplo dado no repasse do usuário. Vale confirmar esse número exato (2) antes da implementação, ou ajustar para outro critério (ex.: 3 eventos, ou um volume mínimo de quantidade consumida)? (Nota v1.1: a ORDEM da cascata de janelas — 30 → 60 → 90 — já foi corrigida e confirmada durante a implementação; esta pendência é só sobre o valor "2" do critério de suficiência, não sobre a ordem.)
...
| 1.1 | 22/09/2026 | Guilherme Scandelari (via uml-use-case-writer) | **Correção de bug de design na cascata de janelas de histórico (RN-03), confirmada pelo usuário durante a implementação.** O desenho original (v1.0) descrevia a cascata como 90 → 60 → 30 dias, tentando a janela mais longa primeiro. Ao escrever os testes unitários de `selectConsumptionWindow` (`src/__tests__/projectionService.test.ts`), constatou-se que essa ordem torna o fallback para janelas menores matematicamente inalcançável: como todas as janelas são retroativas (terminam em "hoje"), a janela de 60 dias é sempre um subconjunto da de 90, e a de 30 é sempre um subconjunto da de 60 — logo, se a janela de 90 dias não tiver `MIN_DISTINCT_CONSUMPTION_DATES` (2) datas distintas de consumo, nenhum subconjunto seu (60 ou 30 dias) pode ter mais datas distintas, e o resultado é sempre "90 dias" (se suficiente) ou "dados insuficientes" (se não), nunca "60" ou "30". Corrigido para 30 → 60 → 90 dias (tenta a janela mais recente/estreita primeiro, ampliando apenas quando os dados recentes forem insuficientes), tornando o fallback de fato alcançável. Atualizados: Fluxo Principal (passo 2b), Fluxo Alternativo 7a, RN-03 (regra e justificativa), exemplo de aviso no passo 6, e referências de código (`projectionService.ts`, `HISTORY_WINDOWS_DAYS = [30, 60, 90]`, e o arquivo de testes correspondente). O critério de suficiência em si (2 datas distintas de consumo por janela) **não mudou** — apenas a ordem de avaliação da cascata. Correção já refletida no código e nos testes da branch `feat/uc52-projecao-reposicao-estoque` (commits `06017e8`, `cd4b125`) antes desta atualização do documento. Status mantido como "Aprovado" — não é uma decisão pendente, é a correção de uma decisão já tomada. |
