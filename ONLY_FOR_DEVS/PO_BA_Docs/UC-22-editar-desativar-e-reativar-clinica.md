...
- Nenhuma alteração é feita; mensagem de erro exibida na própria tela (na listagem, via `alert()`; na tela de detalhe, no bloco `error` do formulário — RN-08).
...
2. Sistema exibe `alert(err.message || 'Erro ao desativar clínica')` — um alerta nativo do navegador, diferente do bloco de erro inline usado na tela de detalhe (RN-08).
...
| RN-08 | **[Achado]** O feedback de sucesso/erro da desativação difere entre as duas telas: na tela de detalhe, sucesso mostra uma mensagem inline ("Clínica desativada com sucesso!") e erro mostra um bloco de erro inline; na listagem, sucesso **não mostra nenhuma mensagem** (o único feedback é a lista recarregar com o badge atualizado) e erro usa `alert()` nativo do navegador. | Confirmado por leitura comparada de `handleDeactivate` em `admin/tenants/page.tsx` (sem `success`/`setSuccess`, `alert()` no `catch`) e do equivalente em `admin/tenants/[id]/page.tsx` (RN-04 original, mantida). |
...
| RNF-04 | Inconsistência de padrão de feedback de UI entre as duas telas (RN-08) — a listagem usa `alert()` nativo para erro e nenhuma confirmação de sucesso, enquanto a tela de detalhe usa blocos de mensagem inline para ambos os casos. | Consistência de UI |
...
5. **[RN-08, não confirmado]** Inconsistência de feedback de UI (mensagem inline vs. `alert()`, presença vs. ausência de confirmação de sucesso) entre as duas telas — avaliação de necessidade de padronização não solicitada até o momento.
