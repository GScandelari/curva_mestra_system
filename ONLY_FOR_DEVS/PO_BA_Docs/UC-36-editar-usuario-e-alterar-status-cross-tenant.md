...
- **[CORRIGIDO — commit `c3f18d4`]**: ao desativar um usuário ("Status: Inativo", `disabled: true`), se esse usuário tentar logar depois, agora vê a mensagem amigável em PT-BR ("Esta conta foi desativada. Entre em contato com o suporte.") em vez da mensagem crua do SDK do Firebase Auth — ver RN-10.
...
15. Caso de uso é concluído com sucesso — as custom claims e o estado `disabled` da conta do usuário-alvo agora refletem exatamente a função e o status escolhidos: se a "Função" foi alterada, o acesso do usuário (via `ProtectedRoute` e regras do Firestore de outras coleções, que leem a claim) muda imediatamente na próxima verificação do token; se o "Status" foi mudado para "Inativo", o usuário não consegue mais logar (conta `disabled: true` no Firebase Auth) — **[CORRIGIDO, commit `c3f18d4`]** e agora vê uma mensagem amigável em PT-BR ao tentar (RN-10, corrigida).
...
### 8d. [CORRIGIDO — commit `c3f18d4`] Usuário desativado por este UC tenta logar (`/login`) — mensagem de erro agora traduzida
...
5. Sistema exibe ao usuário: "Esta conta foi desativada. Entre em contato com o suporte." — mensagem amigável em PT-BR, em vez do texto cru e em inglês do SDK. Mesma correção aplicada simultaneamente em UC-29 (RN-09), mesma tela de login.

**Comportamento anterior (histórico, antes da correção):** o sistema exibia a mensagem crua e em inglês do SDK (ex.: `Firebase: The user account has been disabled by an administrator. (auth/user-disabled)`), pois `translateFirebaseError` não tinha nenhum tratamento para `user-disabled` e caía no `default`. Este code path só se tornara alcançável na prática após a correção de RN-02/RN-03 (`bugfix/sync-user-claims-on-edit`), que passou a desabilitar a conta de verdade no Firebase Auth.
...
| RN-10 | **[CORRIGIDO — commit `c3f18d4`]** Desde que definir "Status: Inativo" por este UC passou a desabilitar de fato a conta no Firebase Auth (`disabled: true`, RN-03), o código de erro `auth/user-disabled`, lançado por `signInWithEmailAndPassword` ao tentar logar com essa conta, passou a ser efetivamente alcançável em `/login`. A partir do commit `c3f18d4`, esse código passou a ser tratado: `signIn` (`src/hooks/useAuth.ts`) agora retorna `{ success: false, error: error.code }` em vez de `error.message`; `translateFirebaseError` (`src/app/(auth)/login/page.tsx`) ganhou um novo `if (error.includes('user-disabled')) { return 'Esta conta foi desativada. Entre em contato com o suporte.'; }`, adicionado antes do `default`. O usuário desativado agora vê essa mensagem amigável em PT-BR, em vez do texto cru e em inglês do SDK. Mesmo achado corrigido simultaneamente em UC-29 (RN-09), mesma causa raiz e mesma correção, pois a tela de login é compartilhada. **Comportamento anterior (histórico):** o usuário via a mensagem crua do SDK (`Firebase: The user account has been disabled by an administrator. (auth/user-disabled)`); achado só se tornara alcançável na prática após a correção de RN-02/RN-03 (`bugfix/sync-user-claims-on-edit`). | Corrigido por leitura literal do diff do commit `c3f18d4` em `src/hooks/useAuth.ts` (retorno `error.code`) e `src/app/(auth)/login/page.tsx` (`translateFirebaseError`, novo `if` para `user-disabled`). |
...
- **UC-29 (Editar, Suspender e Reativar Consultor)** — achado de UX compartilhado, **[CORRIGIDO — commit `c3f18d4`]** (RN-10 aqui / RN-09 lá): a tela de login (`login/page.tsx`) não tratava o código `auth/user-disabled` do Firebase Auth para nenhuma conta desabilitada, seja usuário de clínica (status "Inativo" por este UC) ou consultor (suspenso por UC-29) — mesma causa raiz, corrigida numa única vez (mesma tela de login serve os dois fluxos), pois `signIn` (`useAuth.ts`) passou a retornar `error.code` e `translateFirebaseError` ganhou o `case` faltante.
- **UC-04 (Fazer Login com Redirecionamento por Papel)** — dono da tela `/login` onde a correção de RN-10 foi aplicada; não documentado formalmente lá nesta rodada (correção registrada apenas em UC-29/UC-36, por ter sido descoberta e corrigida no contexto destes dois UCs).
...
- `src/hooks/useAuth.ts` (`signIn` — **[CORRIGIDO, commit `c3f18d4`]** agora retorna `error.code` em vez de `error.message`, ver RN-10), `src/components/auth/ProtectedRoute.tsx` (fonte de verdade de acesso: custom claims, nunca o documento Firestore)
- `src/app/(auth)/login/page.tsx` (`translateFirebaseError` — **[CORRIGIDO, commit `c3f18d4`]** ganhou tratamento para `user-disabled`, ver RN-10)
...
- Commit `c3f18d4` (traduz `auth/user-disabled` na tela de login, RN-10)
...
| 1.1.2 | 26/07/2026 | Guilherme Scandelari | **Correção de bug (commit `c3f18d4`)**: RN-10 corrigida — `signIn` (`src/hooks/useAuth.ts`) passou a retornar `error.code` em vez de `error.message`; `translateFirebaseError` (`src/app/(auth)/login/page.tsx`) ganhou `if (error.includes('user-disabled')) { return 'Esta conta foi desativada. Entre em contato com o suporte.'; }`. Usuário desativado agora vê mensagem amigável em PT-BR ao tentar logar, em vez do texto cru em inglês do SDK. Correção feita simultaneamente em UC-29 (RN-09), mesma tela de login. Seções 4.1, 6 (passo 15), 8 (Fluxo 8d), 9 (RN-10), 12, 13 e 14 (item 4) atualizadas. |
