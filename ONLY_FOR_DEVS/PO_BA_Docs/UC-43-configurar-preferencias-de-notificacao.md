...
| RN-07 | Diferente de UC-15 (`stock_limits`, sem regra dedicada no Firestore), a coleção `tenants/{tenantId}/settings/notifications` **tem** regra dedicada: leitura permitida a qualquer usuário do tenant, mas escrita (`allow write`) restrita a `role == 'clinic_admin'` — consistente com o gate de UI desta tela. **[Ressalva adicionada em 25/07/2026]**: essa regra dedicada, embora presente no arquivo, é hoje inefetiva na prática pelo mesmo motivo documentado em UC-44/RN-02 — a regra genérica de subcoleção do tenant (`tenants/{tenantId}/{document=**}`) já concede escrita a qualquer usuário do tenant via semântica OR do Firestore (múltiplas regras que casam o mesmo caminho são combinadas com OR, não com "mais específica vence"). Ou seja: um `clinic_user` poderia, tecnicamente, alterar as configurações de notificação chamando o Firestore diretamente, fora da UI que só exibe esta tela para `clinic_admin`. A proteção real hoje é só o gate de UI. Ver achado de severidade Alta consolidado no mapa de bugs (UC-13-RN-09/UC-15-RN-07) para o contexto completo e a decisão de adiar a correção. | Confirmado por leitura de `firestore.rules`, linhas 77-86 (regra dedicada) e linhas 58-67 (regra genérica de subcoleção do tenant que torna a regra dedicada inefetiva). |
| RN-08 | **[Divergência confirmada entre marketing e produto]** A landing page (`ONLY_FOR_DEVS/new_landing_page/sections-product.jsx`, módulo `id: "inventario"` do array `MODULES`) promete, na `desc` do módulo (linha 96), "**Alertas automáticos de vencimento**" e, em um dos `bullets` (linha 99), "**Validade com alertas de 60/30/15 dias**". O que esta tela efetivamente permite configurar é um único campo `expiry_warning_days` (padrão `30`, ver fluxo 7a) — não três estágios fixos de 60/30/15 — e nenhuma automação: salvar aqui não dispara nenhum agendamento (RN-04 deste UC); os valores só têm efeito na próxima execução manual de UC-42. Ver também UC-42/RN-08 (mesma divergência, do ponto de vista da execução dos checks). | Confirmado por leitura literal de `sections-product.jsx` (linhas 96 e 99) e releitura de `ClinicSettingsPage` e `notificationService.ts` (`DEFAULT_NOTIFICATION_SETTINGS.expiry_warning_days: 30`, campo único, sem estrutura de múltiplos limiares) — reconfirmando RN-04 deste UC (nenhuma chamada a `alertTriggers.ts` a partir deste fluxo). |
...
- `firestore.rules` (linhas 77-86 — regra dedicada de `settings/notifications`; linhas 58-67 — regra genérica de subcoleção do tenant que torna a regra dedicada de RN-07 inefetiva na prática, ver ressalva)
...
5. **[Divergência marketing x produto, requer decisão]** RN-08 — a landing page promete "alertas automáticos de vencimento" com escalonamento "60/30/15 dias" (`sections-product.jsx`, linhas 96 e 99), mas esta tela só permite configurar um único limiar (`expiry_warning_days`, padrão 30 dias) e nenhuma automação (RN-04). Duas opções não excludentes entre si foram identificadas, mas nenhuma foi escolhida por este documento: (a) evoluir este UC para suportar de fato três estágios configuráveis (60/30/15) e acionar automação real (Scheduled Function, ver UC-42/RN-08); ou (b) ajustar o texto da landing page para refletir o limiar único e o disparo manual hoje existentes. Decisão de produto pendente — não decidida aqui.
6. **[Ressalva de precisão factual adicionada em 25/07/2026 — não é decisão nova]** A regra dedicada de RN-07 (escrita em `settings/notifications` restrita a `clinic_admin`) é hoje inefetiva na prática, pois a regra genérica de subcoleção do tenant já permite a mesma escrita a qualquer usuário do tenant via semântica OR do Firestore. Este achado foi registrado com severidade Alta no mapa de bugs (`_MAPA-DE-BUGS-E-MELHORIAS.md`, UC-13-RN-09/UC-15-RN-07), com correção adiada por decisão do usuário — corrigir exigiria alterar a regra genérica usada por todo o sistema multi-tenant, não apenas esta subcoleção.
