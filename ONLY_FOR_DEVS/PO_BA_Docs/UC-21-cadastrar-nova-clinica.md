...
- Um usuário é criado no Firebase Auth com a senha exatamente como digitada pelo System Admin (`emailVerified: false`) e custom claims (`tenant_id`, `role: "clinic_admin"`, `is_system_admin: false`, `active: true`).
- Um documento é criado em `users` (Firestore).
...
11. API cria o documento do usuário em `users` (Firestore: `tenant_id`, `email`, `full_name`, `phone`, `role: "clinic_admin"`, `active: true`) — se falhar, deleta tanto o usuário Auth quanto o tenant, e retorna erro (Fluxo de Exceção 8e).
12. API define os custom claims (`tenant_id`, `role: "clinic_admin"`, `is_system_admin: false`, `active: true`) — se falhar, apenas registra o erro no log, sem desfazer nada já criado.
...
| RN-05 | **[Confirmado, gap relevante]** Apesar do aviso na tela ("O usuário deverá alterá-la no primeiro acesso") e no e-mail ("Por favor, altere sua senha no primeiro acesso"), não existe nenhum mecanismo técnico que force essa troca — a claim `requirePasswordChange` (usada em UC-06) nunca é setada por este fluxo. O novo `clinic_admin` pode logar e usar o sistema indefinidamente com a senha temporária original, sem nenhuma obrigação real de trocá-la. | Confirmado pela ausência de qualquer referência a `requirePasswordChange` em `/api/tenants/create/route.ts`. |
...
| RNF-02 | A falha ao definir custom claims não é revertida — apenas logada — podendo deixar um usuário criado sem os claims corretos (`tenant_id`/`role`) em um cenário raro de falha parcial. | Confiabilidade |
...
- **UC-06 (Trocar Senha Obrigatória no Primeiro Acesso)** **não** se aplica a usuários criados por este UC, já que `requirePasswordChange` nunca é setada aqui (RN-05) — diferente de outros mecanismos de criação de usuário administrativo já documentados.
...
1. **[Confirmado, gap relevante]** RN-05 — nenhuma obrigatoriedade técnica de troca de senha para usuários criados por este caminho, apesar do aviso textual.
