...
## 13. Referências
- `src/app/(admin)/admin/tenants/new/page.tsx` (linhas alteradas pelo commit `66c75fa` — remoção da senha do template padrão do e-mail, de `getPreviewEmail()` e do texto de ajuda do campo de senha, RN-03)
...
3. **[Observação]** RN-09 — e-mail de notificação interna *hardcoded*, sem configuração; `plan_id` nunca é definido por nenhum dos dois fluxos de criação de tenant.
4. **[Observação de manutenibilidade]** RNF-03 — `tenantService.ts` parece ser código legado apontando para uma Cloud Function que não existe no projeto.
5. **[Observação]** RN-07 — impossibilidade de retroceder no wizard sem cancelar tudo.
