...
flowchart LR
    ...
    UC21 -.->|enfileira e-mail de boas-vindas\n(sem a senha, v1.2)| EmailQueue
...
- Um e-mail de boas-vindas — assunto e corpo totalmente customizáveis pelo System Admin — é enfileirado em `email_queue`. **[CORRIGIDO em v1.2, commit `66c75fa`]** A senha em texto plano **não faz mais parte** do template padrão do e-mail: a linha `- Senha temporária: {{temp_password}}` foi removida, e o aviso de troca de senha foi substituído por "Sua senha de acesso não é enviada por e-mail — foi definida pelo System Admin durante o cadastro. Você será solicitado a trocá-la no primeiro acesso ao sistema." Mesmo que o System Admin edite manualmente o corpo do e-mail (campo textarea livre) e digite `{{temp_password}}`, o placeholder **não é mais substituído** pela senha real em `getPreviewEmail()` — defesa ativa contra reintrodução do vazamento.
...
3. **Passo 2 ("Administrador"):** System Admin informa nome completo, e-mail e telefone do administrador inicial da clínica, e digita manualmente uma "Senha Temporária" (mínimo 6 caracteres, campo de texto simples, visível em tela) com o aviso **"Esta senha não é enviada por e-mail — comunique-a ao administrador por fora do sistema. Ele será solicitado a trocá-la no primeiro acesso." — `[CORRIGIDO em v1.2, commit 66c75fa]`, texto anterior era "Esta senha será enviada por e-mail. O usuário deverá alterá-la no primeiro acesso."**; clica em "Próximo" (valida nome, e-mail e senha).
4. **Passo 3 ("E-mail de Boas-Vindas"):** sistema pré-preenche um assunto e um corpo de e-mail padrão, com placeholders `{{admin_name}}`, `{{clinic_name}}`, `{{admin_email}}` (o placeholder `{{temp_password}}` foi removido do template padrão em v1.2 e não é mais substituído por `getPreviewEmail()` mesmo se reintroduzido manualmente — RN-03, `[CORRIGIDO]`); System Admin pode editar livremente o assunto e o corpo inteiro; pode clicar em "Visualizar Preview" a qualquer momento para ver o e-mail com os placeholders já substituídos pelos dados reais informados.
5. System Admin clica em "Criar Clínica e Enviar E-mail".
6. Sistema chama `createTenant({...dados da clínica, admin_name, admin_email, admin_phone, temp_password, welcome_email: {subject, body, send: true}}, token)`.
7. Como há dados de administrador presentes, o service chama `POST /api/tenants/create` com o header `Authorization: Bearer {token}` (token obtido via `user.getIdToken()` na página, antes da chamada) em vez de gravar diretamente no Firestore (RN-10). **[CORRIGIDO em v1.3, commit `a9449a3`]** Até essa correção, esse header nunca era enviado — ver RN-08.
...
13. API adiciona um documento à fila `email_queue` (`to: admin_email`, `subject`, `body` — já com os placeholders substituídos no cliente antes do envio, sem `{{temp_password}}` — RN-03, `[CORRIGIDO]` —, `status: "pending"`), sem um campo `type` (diferente do `type: "welcome_approval"` usado por UC-02).
...
2. Sistema abre um Dialog mostrando destinatário, assunto e corpo já com os placeholders substituídos (sem a senha — RN-03, `[CORRIGIDO]`).
...
4. **[Histórico — RESOLVIDO em v1.3, commit `a9449a3`]** Até essa correção, este fluxo de exceção era acionado em **toda** tentativa real de uso desta tela por um System Admin: o client nunca enviava o header `Authorization`, então a rota sempre retornava 401 "Não autorizado" já na primeira checagem, antes mesmo de avaliar `is_system_admin`. Ver RN-08 para o achado completo.
...
| RN-03 | **[CORRIGIDO em v1.2, commit `66c75fa`]** A senha inicial do administrador continua sendo escolhida manualmente pelo próprio System Admin (campo de texto livre, visível em tela, mínimo 6 caracteres) — diferente de UC-02, que gera uma senha temporária aleatória e opaca (`crypto.randomBytes`) e nunca a exibe para ninguém, exigindo redefinição via link do Firebase. **Até a v1.1, essa senha era enviada por e-mail em texto plano** dentro do corpo do e-mail de boas-vindas — achado de severidade Alta. O commit `66c75fa` removeu a linha `- Senha temporária: {{temp_password}}` do template padrão do e-mail (`emailBody`) e o texto de aviso associado, substituindo-o por "Sua senha de acesso não é enviada por e-mail — foi definida pelo System Admin durante o cadastro. Você será solicitado a trocá-la no primeiro acesso ao sistema."; removeu também a linha `.replace(/{{temp_password}}/g, ...)` de `getPreviewEmail()`, de forma que o placeholder `{{temp_password}}` não é mais substituído pela senha real mesmo que o System Admin o digite manualmente no corpo editável do e-mail (defesa contra reintrodução do vazamento); e corrigiu o texto de ajuda ao lado do campo "Senha Temporária" na tela (de "Esta senha será enviada por e-mail..." para "Esta senha não é enviada por e-mail — comunique-a ao administrador por fora do sistema..."). O mecanismo real de segurança (`requirePasswordChange: true`, RN-05) já existia desde o commit `91eb37e` (v1.1) e não precisou de alteração — a correção da RN-03 elimina o canal de vazamento em texto plano, complementando aquele mecanismo. | Corrigido conforme diff de `src/app/(admin)/admin/tenants/new/page.tsx` no commit `66c75fa` — remoção da linha do template, da substituição do placeholder em `getPreviewEmail()`, e do texto de ajuda do campo de senha. |
...
| RN-08 | **[Bug funcional, severidade Alta — RESOLVIDO em v1.3, commit `a9449a3`]** A rota `/api/tenants/create` sempre exigiu e validou, do lado do servidor, um Bearer token real (com `is_system_admin === true`) antes de qualquer gravação — isso nunca mudou, e é o que de fato diferencia esta rota da rota de aprovação de UC-02 (`/api/access-requests/[id]/approve`), que não valida nenhum Bearer token (gap já registrado em UC-02, RNF-01). **Porém**, a v1.0/v1.1/v1.2 desta seção registravam isso como "fato confirmado" com base apenas na leitura do servidor (`route.ts`) — sem nunca ter confirmado que o client de fato enviava esse token. Havia um bug real: até o commit `a9449a3` (24/08/2026), `tenantServiceDirect.createTenant()` fazia o `fetch('/api/tenants/create', ...)` **sem nenhum header `Authorization`**. Como resultado, **toda** chamada real de um `system_admin`, feita através da única tela que usa esta função (`/admin/tenants/new`), era rejeitada já na primeira checagem da rota (`if (!authHeader || !authHeader.startsWith('Bearer ')) return 401 'Não autorizado'`) — antes mesmo de a rota chegar a checar `is_system_admin`. Ou seja: nenhum System Admin conseguia, na prática, cadastrar uma clínica com administrador por este caminho — o único fluxo real que esta função atende (confirmado pelo próprio RN-10: o branch sem admin é inalcançável a partir da UI). O bug foi descoberto por teste manual em `dev-gscandelari.web.app` (recebeu "Não autorizado"), não por auditoria de código, e corrigido no commit `a9449a3` (branch `bugfix/tenant-create-missing-auth-header`, já mergeada em `develop`): `createTenant()` passou a receber um parâmetro `token: string` e enviar `Authorization: Bearer {token}`; a página `admin/tenants/new/page.tsx` passou a obter o token via `useAuth().user.getIdToken()` antes de chamar a função (único call site, confirmado por grep). | Confirmado por leitura literal de `route.ts` (a validação de servidor sempre existiu) e por leitura do diff e do estado atual de `tenantServiceDirect.ts`/`page.tsx` (header ausente antes do commit `a9449a3`, presente depois). |
...
- **UC-28 (Cadastrar Consultor)** recebeu, no mesmo commit `66c75fa`, a correção equivalente de RN-03 (remoção da senha em texto plano do e-mail de boas-vindas do consultor).

---

## 13. Referências
- `src/app/(admin)/admin/tenants/new/page.tsx` (linhas alteradas pelo commit `66c75fa` — remoção da senha do template padrão do e-mail, de `getPreviewEmail()` e do texto de ajuda do campo de senha, RN-03; e pelo commit `a9449a3` — `handleSubmit` passou a obter o token via `user.getIdToken()` e repassá-lo para `createTenant`, RN-08)
- `src/lib/services/tenantServiceDirect.ts` (`createTenant` — alterado pelo commit `a9449a3`: passou a receber um parâmetro `token: string` e enviar o header `Authorization: Bearer {token}`, corrigindo a ausência total desse header nas chamadas anteriores, RN-08)
...
2. **[RESOLVIDO em v1.2 — commit `66c75fa`]** RN-03 — implementado: a senha temporária deixou de ser enviada em texto plano por e-mail (template padrão, preview e texto de ajuda da tela corrigidos); o placeholder `{{temp_password}}` foi removido de `getPreviewEmail()` como defesa contra reintrodução do vazamento.
3. **[RESOLVIDO em v1.3 — commit `a9449a3`]** RN-08 — achado de bug real (não apenas uma observação de contraste com UC-02): a validação de Bearer token na rota `/api/tenants/create` sempre existiu do lado do servidor, mas o client (`tenantServiceDirect.createTenant`) nunca enviava o header `Authorization` até este commit — nenhum System Admin conseguia, na prática, cadastrar uma clínica com administrador pela tela `/admin/tenants/new`. Descoberto por teste manual (não por auditoria de código); corrigido enviando o token obtido via `user.getIdToken()`.
4. **[Observação]** RN-09 — e-mail de notificação interna *hardcoded*, sem configuração; `plan_id` nunca é definido por nenhum dos dois fluxos de criação de tenant.
5. **[Observação de manutenibilidade]** RNF-03 — `tenantService.ts` parece ser código legado apontando para uma Cloud Function que não existe no projeto.
6. **[Observação]** RN-07 — impossibilidade de retroceder no wizard sem cancelar tudo.
...
| 1.2 | 24/07/2026 | Guilherme Scandelari | Correção de severidade Alta (commit `66c75fa`): removida a senha temporária em texto plano do e-mail de boas-vindas — template padrão (`emailBody`), função `getPreviewEmail()` (placeholder `{{temp_password}}` não é mais substituído, mesmo se reintroduzido manualmente) e texto de ajuda ao lado do campo "Senha Temporária" na tela. RN-03 marcada como `[CORRIGIDO]`; seções 4.1, 6 (passos 3/4/13), 7b e 13 atualizadas. Registrado, na seção 12, que UC-28 recebeu a correção equivalente no mesmo commit. |
| 1.3 | 21/09/2026 | Guilherme Scandelari | Correção fatual de RN-08 (bug funcional real, severidade Alta): a afirmação original ("a API exige e valida um Bearer token real... antes de qualquer gravação") só havia sido verificada do lado do servidor (`route.ts`) — nunca se confirmou que o client de fato enviava esse token. Na prática, `tenantServiceDirect.createTenant()` nunca enviava o header `Authorization`, e toda chamada real de um System Admin era rejeitada com 401 já na primeira checagem da rota, antes mesmo de chegar à checagem de `is_system_admin` — nenhum System Admin conseguia cadastrar uma clínica com administrador por este caminho. Achado descoberto por teste manual em `dev-gscandelari.web.app` (não por auditoria de código) e corrigido no commit `a9449a3` (branch `bugfix/tenant-create-missing-auth-header`, 24/08/2026, já mergeada em `develop`): `createTenant()` passou a receber um parâmetro `token` e enviar `Authorization: Bearer {token}`; a página `/admin/tenants/new` passou a obter o token via `useAuth().user.getIdToken()`. RN-08 marcada como `[RESOLVIDO]`; seções 2.1, 6 (passo 7), 8c, 13 e 14 atualizadas. |
