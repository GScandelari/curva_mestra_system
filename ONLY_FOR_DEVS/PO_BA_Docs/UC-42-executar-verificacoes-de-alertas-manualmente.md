...
flowchart LR
    ...
    UC42 -->|cria/consulta, só lotes ativos desde 216b3a0| Firestore
```
...
- **[CORRIGIDO no commit `216b3a0`]** As três varreduras de inventário (`checkExpiringProducts`, `checkExpiredProducts`, `checkLowStock`) agora consideram apenas lotes `active: true` — itens já desativados (ex.: por UC-13) não contribuem mais com quantidade residual para nenhum dos três checks (RN-03).
...
6. **[CORRIGIDO no commit `216b3a0`]** Cada função busca os documentos de `tenants/{tenantId}/inventory` filtrando `where('active', '==', true)` — sem paginação (RN-03; antes, a query não filtrava por `active`, e lotes já desativados por UC-13 também eram contados).
...
|----|-------|----------------|
| RN-01 | Apenas `clinic_admin` pode executar os checks — e, diferente de UC-15 (`stock_limits`), aqui a restrição também é reforçada no Firestore: a regra `match /tenants/{tenantId}/notifications/{notificationId}` exige `role == 'clinic_admin'` para `create` e `delete` (mas não para `read`/`update`, que qualquer usuário do tenant pode fazer). **[Ressalva adicionada em 25/07/2026]**: essa regra dedicada, embora presente no arquivo, é hoje inefetiva na prática — a regra genérica de subcoleção do tenant (`tenants/{tenantId}/{document=**}`) já concede `create`/`delete` a qualquer usuário do tenant via semântica OR do Firestore (múltiplas regras que casam o mesmo caminho são combinadas com OR, não com "mais específica vence"). Ou seja: um `clinic_user` poderia, tecnicamente, criar uma notificação chamando o Firestore diretamente, fora da UI que restringe a execução dos checks a `clinic_admin` (`isAdmin &&`). A proteção real hoje é só o gate de UI. Ver achado de severidade Alta consolidado no mapa de bugs (UC-13-RN-09/UC-15-RN-07) para o contexto completo e a decisão de adiar a correção. | Confirmado por leitura de `AlertsTab.tsx` (`isAdmin &&`) e `firestore.rules` linhas 65-74 (regra dedicada) e linhas 58-67 (regra genérica de subcoleção do tenant que torna a regra dedicada inefetiva). |
...
| RN-03 | **[CORRIGIDO — commit `216b3a0`]** Até esta correção, nenhuma das três funções (`checkExpiringProducts`, `checkExpiredProducts`, `checkLowStock`) filtrava `active: true` ao buscar `tenants/{tenantId}/inventory` — todas somavam/avaliavam também lotes já desativados (ex.: por UC-13), o mesmo padrão catalogado originalmente apenas em UC-15/RN-08 (para `checkLowStock`). Corrigido: as três funções agora usam `query(inventoryRef, where('active', '==', true))` em vez de ler a coleção inteira sem filtro — nenhuma delas soma mais quantidade residual de lotes desativados. | Corrigido por leitura direta de `checkExpiringProducts`, `checkExpiredProducts` e `checkLowStock` (`alertTriggers.ts`), commit `216b3a0` — mesma correção aplicada e documentada em UC-15/RN-08. |
...
|----|-----------|-----------|
| RNF-01 | Cada função de check faz uma varredura completa de `tenants/{tenantId}/inventory` (filtrada por `active: true` desde o commit `216b3a0`) sem paginação; para tenants com muitos itens de estoque, a operação pode ficar lenta e cara em leituras do Firestore (agravado por rodar as três funções em paralelo via `Promise.all`). | Performance |
...
- **UC-15 (Configurar Limite de Estoque Baixo por Produto)** — o limite específico por `codigo_produto`, usado como primeira prioridade no fallback de `checkLowStock`, é configurado lá. O mesmo bug de não filtrar `active: true` (RN-03/RN-08 respectivamente) foi corrigido em ambos os UCs no mesmo commit `216b3a0`.
- **UC-13 (Desativar Item de Estoque com Verificação de Reservas Ativas)** — itens desativados por aquele UC eram somados por `checkLowStock`/considerados por `checkExpiringProducts`/`checkExpiredProducts`, pois nenhuma das três funções filtrava `active: true` (RN-03) — corrigido no commit `216b3a0`.
- **UC-44 (Consultar e Gerenciar Notificações Recebidas)** — via `NotificationBell`, header do `ClinicLayout`, é quem efetivamente exibe, marca como lida e permite excluir as notificações criadas por este caso de uso. O mesmo achado estrutural de regra dedicada inefetiva por semântica OR (RN-01 deste UC) também se aplica à regra de `delete` de UC-44/RN-02.
...
- `src/lib/services/alertTriggers.ts` (`checkExpiringProducts`, `checkExpiredProducts`, `checkLowStock`, `runAllChecks`, `runChecksForAllTenants`; filtro `active: true` adicionado às três primeiras no commit `216b3a0`, RN-03)
...
- `firestore.rules` (linhas 65-86 — regras dedicadas de `notifications` e `settings/notifications`; linhas 58-67 — regra genérica de subcoleção do tenant que torna a regra dedicada de RN-01 inefetiva na prática, ver ressalva)
...
⚠️ Os itens abaixo são achados confirmados por leitura de código que representam decisões de produto pendentes de confirmação — não foram decididos unilateralmente por este documento, exceto os itens já marcados como resolvidos/corrigidos.
...
4. **[Observação, adiada]** RN-05 — como já registrado em UC-15, não há nenhum agendamento automático das verificações; toda a geração de alertas depende de um Clinic Admin acionar esta tela manualmente. Mesma dependência de infraestrutura ainda inexistente, adiada junto com UC-15/RN-05.
...
7. **[RESOLVIDO em v1.4 — commit `216b3a0`]** RN-03 — as três funções de verificação (`checkExpiringProducts`, `checkExpiredProducts`, `checkLowStock`) agora filtram `active: true` na query de inventário; itens desativados (ex.: por UC-13) não são mais somados/considerados por nenhum dos três checks. Mesma correção aplicada em conjunto com UC-15/RN-08.
8. **[Ressalva de precisão factual adicionada em 25/07/2026 — não é decisão nova]** A regra dedicada de RN-01 (`create`/`delete` de notificação restritos a `clinic_admin`) é hoje inefetiva na prática, pois a regra genérica de subcoleção do tenant já permite as mesmas operações a qualquer usuário do tenant via semântica OR do Firestore. Este achado foi registrado com severidade Alta no mapa de bugs (`_MAPA-DE-BUGS-E-MELHORIAS.md`, UC-13-RN-09/UC-15-RN-07), com correção adiada por decisão do usuário — corrigir exigiria alterar a regra genérica usada por todo o sistema multi-tenant, não apenas esta subcoleção.
...
| 1.4 | 25/07/2026 | Guilherme Scandelari | **Correção de bug de severidade Média (commit `216b3a0`)**, cross-referenciada com UC-15/RN-08: RN-03 corrigida — as três funções de verificação (`checkExpiringProducts`, `checkExpiredProducts`, `checkLowStock`) agora filtram `active: true` na query de inventário, deixando de somar/considerar lotes já desativados por UC-13. Seções 4.1, 6 (passos 6 e 9), 9 (RN-03, RN-05 com nota de escopo), 10 (RNF-01), 12, 13 e 14 (novo item 7) atualizadas de acordo. RN-05 (agendamento automático) e RN-06/RN-08 permanecem sem correção, sem alteração de conteúdo além da nota de escopo em RN-05. |
| 1.4.1 | 25/07/2026 | Guilherme Scandelari (via uml-use-case-writer) | **Ressalva de precisão factual, sem correção de código.** RN-01 (seção 9) passou a deixar explícito que a regra dedicada que restringe `create`/`delete` de notificação a `clinic_admin` (`firestore.rules`, linhas 65-74), embora exista no arquivo e não tenha sido alterada, é hoje **inefetiva na prática**: a regra genérica de subcoleção do tenant (`tenants/{tenantId}/{document=**}`, linhas 58-67) já concede as mesmas operações a qualquer usuário do tenant via semântica OR do Firestore (regras que casam o mesmo caminho são combinadas com OR, não com "mais específica vence") — um `clinic_user` poderia, tecnicamente, criar uma notificação chamando o Firestore diretamente. A proteção real hoje é apenas o gate de UI (`isAdmin &&`). Achado cross-referenciado com o item de severidade Alta consolidado no mapa de bugs (UC-13-RN-09/UC-15-RN-07), com correção adiada. Adicionado item 8 na seção 14, atualizadas as referências (seção 13) e a seção 12 (cross-reference com UC-44/RN-02). |
