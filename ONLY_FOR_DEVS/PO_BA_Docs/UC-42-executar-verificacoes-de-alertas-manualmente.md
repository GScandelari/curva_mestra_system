...
- Zero ou mais documentos são criados em `tenants/{tenantId}/notifications` (tipos `expiring` e `low_stock` — ver RN-02 para a exceção do tipo `expired`), cada um com `read: false`.
...
8. `checkExpiredProducts` verifica, item a item, se `dt_validade < hoje`; para cada item elegível sem notificação `expired` não lida existente, **não cria nenhuma notificação real** (RN-02) — apenas incrementa o contador interno `notificationsCreated` e loga no console, dando a falsa impressão de sucesso.
...
### 8c. Produto vencido não gera notificação real (a partir do passo 8)
1. `checkExpiredProducts` encontra um item com `dt_validade < hoje` e nenhuma notificação `expired` não lida existente.
2. A função executa `await getDocs(notificationsRefCreate)` — uma leitura, comentada no código-fonte como `"Placeholder para addDoc"` — e **não grava nenhum documento**.
3. `results.notificationsCreated++` é incrementado mesmo assim, e o console loga `"🚨 Alerta URGENTE criado: ... VENCIDO ..."`.
4. A UI (toast e card de resultados) reporta esse produto como um alerta "criado" — mas nenhuma notificação de produto vencido jamais aparece no `NotificationBell` ou em qualquer outro lugar do sistema, porque não existe helper `createExpiredProductNotification` nem chamada equivalente em toda a base de código (RN-02).
...
| RN-02 | **[Bug confirmado]** `checkExpiredProducts` nunca persiste uma notificação real: o trecho que deveria chamar um criador de notificação (`createExpiredProductNotification` ou equivalente) foi substituído por um `getDocs` comentado como placeholder. `notificationsCreated` é incrementado e o toast reporta sucesso, mas nenhum documento é gravado em `tenants/{tenantId}/notifications` com `type: 'expired'` — confirmado por busca em toda a base de código, sem nenhuma ocorrência de criação desse tipo. | Leitura literal de `alertTriggers.ts` (`checkExpiredProducts`, linhas ~295-300) e busca por `createExpiredProductNotification`/`type: 'expired'` em `src/` e `functions/`, sem nenhum resultado além da definição do tipo em `notification.ts`. |
...
- `src/lib/services/notificationService.ts` (`getNotificationSettings`, `createExpiringProductNotification`, `createLowStockNotification`)
...
2. **[Bug confirmado, requer decisão]** RN-02 — `checkExpiredProducts` não persiste nenhuma notificação real (apenas conta e loga). Isso deveria ser corrigido (implementando um `createExpiredProductNotification` de fato) ou é um comportamento aceito/conhecido?
