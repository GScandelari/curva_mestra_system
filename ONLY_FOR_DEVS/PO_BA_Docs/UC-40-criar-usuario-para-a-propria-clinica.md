...
2. Sistema executa em paralelo `loadUsers()` — consulta `collection('users')` com `where('tenant_id', '==', tenantId)` e `orderBy('created_at', 'desc')`, autorizada pela regra `allow list` do Firestore, que desde o commit `eb12c91` também exige `resource.data.tenant_id == request.auth.token.tenant_id` na própria regra (RN-04, `[CORRIGIDO]`) — e `loadTenantInfo()` — lê `tenants/{tenantId}.max_users` (padrão `5`).
...
| RN-04 | **[CORRIGIDO no commit `eb12c91` — UC-40-RN-04]** Antes: a regra `allow list` de `users/{userId}` (`firestore.rules`, linhas 105-109) autorizava qualquer `clinic_admin` autenticado a executar uma consulta de listagem sobre **toda** a coleção `users`, exigindo apenas `request.auth.token.role == 'clinic_admin' && request.auth.token.tenant_id != null` — a condição não referenciava `resource.data.tenant_id` em nenhum momento, apesar do comentário no próprio arquivo de regras dizer "A query deve ter where('tenant_id', '==', <seu_tenant_id>)". Esse filtro era garantido **apenas por convenção no código-cliente** (o `where('tenant_id', '==', tenantId)` que `loadUsers()` aplica em `UsersTab.tsx`), não pela regra em si — uma consulta manual ao SDK do cliente sem esse filtro (ex.: via console do navegador, autenticado como `clinic_admin`) listaria, em tese, usuários de **qualquer** clínica do sistema. Agora: a regra passou a exigir também `&& resource.data.tenant_id == request.auth.token.tenant_id`. Como o Firestore valida operações de `list` pela consulta como um todo (não filtra documento a documento), essa cláusula força a própria query a estar filtrada por `tenant_id` — sem esse filtro, a operação inteira é rejeitada pelo Firestore. Confirmado, antes de implementar a correção, que todo consumo client-side já filtrava por `tenant_id` (`src/lib/services/userManagementService.ts`, 3 ocorrências; `UsersTab.tsx`), então o aperto da regra não quebrou nenhum fluxo legítimo — apenas fechou a brecha teórica. | Correção confirmada por leitura do commit `eb12c91` (`firestore.rules`, `match /users/{userId}`, regra `allow list`). |
...
|----|-----------|-----------|
| RNF-01 | **[Mitigado no commit `eb12c91`]** RN-04 era um risco de segurança/isolamento multi-tenant relevante: a regra do Firestore não impunha, por si só, que um `clinic_admin` só conseguisse listar usuários da própria clínica — dependia inteiramente do código-cliente aplicar o filtro correto. Desde o commit `eb12c91`, a própria regra `allow list` exige `resource.data.tenant_id == request.auth.token.tenant_id`, fechando essa brecha independentemente de qualquer convenção no código-cliente. | Segurança / Multi-tenant |
...
- `firestore.rules` (`match /users/{userId}`, regra `allow list` — RN-04, corrigida no commit `eb12c91`)
- `src/types/index.ts` (`User`, `UserRole`, `CustomClaims`)
- Commit da correção: `eb12c91` (`fix: dois itens de alta severidade (UC-40, UC-44)`) — regra `allow list` de `users/{userId}` passa a exigir `resource.data.tenant_id == request.auth.token.tenant_id` (RN-04)

---

## 14. Perguntas em Aberto / Decisões Pendentes

1. **[RESOLVIDO no commit `eb12c91` — RN-04]** A regra `allow list` de `users/{userId}` não impunha, na própria regra, que a consulta filtrasse por `tenant_id` — isso era garantido só por convenção no código-cliente (`UsersTab.tsx`). Corrigido: a regra passou a exigir explicitamente `resource.data.tenant_id == request.auth.token.tenant_id`, o que força a própria query a estar filtrada por `tenant_id` (sem esse filtro, o Firestore rejeita a operação inteira). Confirmado que nenhum fluxo legítimo existente foi quebrado pela correção.
...
| 1.1 | 19/07/2026 | Guilherme Scandelari (via uml-use-case-writer) | RN-04 marcada como `[CORRIGIDO]`, citando o commit `eb12c91` ("fix: dois itens de alta severidade (UC-40, UC-44)") — a regra `allow list` de `users/{userId}` em `firestore.rules` passou a exigir também `resource.data.tenant_id == request.auth.token.tenant_id`, fechando o risco de isolamento multi-tenant identificado na v1.0 (a query de listagem agora precisa estar filtrada por `tenant_id`, ou é rejeitada inteira pelo Firestore, já que regras de `list` validam a consulta como um todo, não documento a documento). Confirmado que todo consumo client-side já filtrava por `tenant_id` antes da correção, então nenhum fluxo legítimo foi quebrado. Seções 6 (passo 2), 9 (RN-04), 10 (RNF-01), 13 (referências) e 14 (item 1, marcado RESOLVIDO) atualizadas de acordo. |
