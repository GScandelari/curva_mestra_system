...
**Comportamento anterior (histórico, antes da correção):** a API não verificava `tenant.active` em nenhum ponto — a criação prosseguia normalmente mesmo com a clínica desativada, deixando a desativação em cascata de UC-22 incompleta (afetava apenas os usuários já existentes no momento da suspensão, nunca bloqueava a criação de novos).
...
- **UC-21 (Cadastrar Nova Clínica)**, **UC-22 (Editar, Desativar e Reativar Clínica)** e **UC-23 (Vincular, Alterar e Remover Consultor de uma Clínica)** — mesma tela (`admin/tenants/[id]/page.tsx`); este UC-39 fecha a última ação de negócio pendente daquela tela, resolvendo a pendência registrada em UC-22 (seção 14, item 3). **[Atualizado — commit `70a38d7`]** A checagem de `tenant.active` (RN-05) também fecha uma lacuna da desativação em cascata de UC-22 — que antes só afetava usuários já existentes no momento da suspensão, nunca bloqueava a criação de novos usuários por este caminho.
